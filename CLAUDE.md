...
- `.section-title` is one shared base rule (20px desktop, **17px under the 600px breakpoint**; IBM Plex Sans Hebrew 300 from Google Fonts + `line-height: 1.5`; only weight 300 is loaded). No **per-page** font-size/weight overrides, with **one named exception**: `#page-16 .section-title` (@fold17's credits headline) is 36px desktop / 26px mobile. Any other differently-sized title at the same viewport width is a regression. On the scrolling cards the title and the frame are the *same* `<h2>`, so `.text-card-frame`'s `margin: 0 auto` already zeroes the base rule's bottom margin — see [Architecture](wiki/Architecture.md).
