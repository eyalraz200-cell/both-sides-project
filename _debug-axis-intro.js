/* SCAFFOLDING — _debug-axis-intro.js. Delete this file and its <script> tag once
   the value is baked into page7.js.

   MOBILE ONLY (`viewport: 'mobile'` below): the knob it drives
   (P7_AXIS_INTRO_AT_MOBILE) is mobile-scoped, and the panel's own chrome would
   distort the layout being judged. The template's gate WAITS for the breakpoint
...
(function () {
  ...
  var CONFIG = {
    ...
    goTo: '#page-7',
    goLabel: '@fold8 fly',
    // @fold8 is where the fly that gates the wipe happens — park just above it
    // so a scroll down replays the build-in from scratch each time.
    onGo: function (el) {
      if (!el) return;
      window.scrollTo({ top: Math.max(0, el.offsetTop - window.innerHeight * 0.6), behavior: 'smooth' });
    },

    sliders: [
      { key: 'at', label: 'draw-in trigger point', min: 0, max: 1, step: 0.02, value: 0,
        source: 'page7.js P7_AXIS_INTRO_AT_MOBILE' },
      { key: 'ms', label: 'wipe duration', min: 200, max: 6000, step: 50, value: 2800,
        source: 'page7.js P7_AXIS_INTRO_DURATION' }
    ],

    toggles: [
      { key: 'm', id: 'marks', label: 'show trigger mark', on: true }
    ],

    // The knob is a THRESHOLD on fold9FlyTrigger's raw progress, so it only takes
    // effect on the next crossing — scroll back above @fold8 and down again to
    // see a change. Un-latching the intro here makes that replay immediate.
    apply: function (v, mode, tab, on) {
      if (typeof P7_AXIS_INTRO_AT_MOBILE === 'undefined') return;
      P7_AXIS_INTRO_AT_MOBILE = v.at;
      P7_AXIS_INTRO_DURATION  = v.ms;
      window.__axisIntroMarks = !!on.marks;
    ...
    summary: function (v) {
      return [
        'page7.js:',
        '  P7_AXIS_INTRO_AT_MOBILE = ' + v.at + '   // fraction of fold9FlyTrigger',
        '  P7_AXIS_INTRO_DURATION  = ' + v.ms
      ].join('\n');
    }
  };
