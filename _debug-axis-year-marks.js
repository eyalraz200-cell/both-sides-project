...
(function () {
  // ---------------------------------------------------------------- CONFIG --
  var CONFIG = {
    title: 'axis-year-marks',
    label: 'year marks on the timeline axis',
    fold: '@fold9',
    viewport: 'desktop',
    remoteOnly: true,
    goTo: '#page-8',
    goLabel: '@fold9',
    modes: [
      { key: '1', id: 'ring', label: 'hollow dots (today)' },
      { key: '2', id: 'line', label: 'short horizontal lines' }
    ],
    mode: 'ring',
    sliders: [
      { key: 'len', label: 'how long each year line is (px)', min: 2, max: 24, step: 0.5,
        value: 8, ship: 8, modes: ['line'], source: 'page7.js P7_AXIS_YEAR_LINE_LEN' },
      { key: 'alpha', label: 'how dark the year lines are (before a year is reached)', min: 0, max: 1, step: 0.01,
        value: 0.22, ship: 0.22, modes: ['line'], source: 'page7.js P7_AXIS_YEAR_LINE_ALPHA' }
    ],
    apply: function (v, mode) {
      if (typeof P7_AXIS_YEAR_MARK === 'undefined') return;
      P7_AXIS_YEAR_MARK = mode === 'line' ? 'line' : 'ring';
      P7_AXIS_YEAR_LINE_LEN = v.len;
      P7_AXIS_YEAR_LINE_ALPHA = v.alpha;
      if (typeof draw === 'function') draw();
    },
    summary: function (v, mode) { return 'P7_AXIS_YEAR_MARK = ' + (mode === 'line' ? "'line'" : "'ring'"); }
  };
  // ------------------------------------------------------------ END CONFIG --
