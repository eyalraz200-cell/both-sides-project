/* SCAFFOLDING — _debug-fold-timings-mobile-v3.js (the panel finds a harness by
   _debug-<title>.js, so the file name and `title` must match). Delete this file and its entry in
   project.html's dev loader once the durations are baked.

   MOBILE ONLY. One slider per animation from the opening to the timeline, each
   the animation's FULL length in ms — every phase inside it scales by the same
   ratio, so its internal rhythm is kept. Baking must go into MOBILE-scoped
   constants (split a shared one into a *_DESKTOP / *_MOBILE pair).
   GENERATED — desktop and mobile come from one table. */
(function () {
  // ---------------------------------------------------------------- CONFIG --
  var CONFIG = {
    title: 'fold-timings-mobile-v3',
    remoteOnly: true,
    viewport: 'mobile',
    label: 'how long each animation takes, opening to timeline (mobile)',
    fold: 'opening → timeline',
    ...
    sliders: [
      {
            "key": "hero",
            "label": "1 · Opening: title rises, dots pop in, logo fades in",
            "min": 100,
            "max": 8000,
            "step": 50,
            "value": 3114,
            "source": "js/fold1-intro.js — scales: PAGE0_TITLE_MS_MOBILE, PAGE0_ROW_STAGGER_MS_MOBILE, PAGE0_POP_MS_MOBILE, PAGE0_LOGO_FADE_MS_MOBILE"
      },
      {
            "key": "camps",
            "label": "2 · Dots fly into the two camps, camp names type in",
            "min": 100,
            "max": 5000,
            "step": 50,
            "value": 1600,
            "source": "js/groups.js — scales: FOLD2_ENTRANCE_MS_MOBILE"
      },
      {
            "key": "groups",
            "label": "3 · Extra rects shrink away, group names type in",
            "min": 100,
            "max": 4000,
            "step": 50,
            "value": 1600,
            "source": "js/groups.js — scales: FOLD3_BEAT_MS_MOBILE.shrink.len, FOLD3_BEAT_MS_MOBILE.align.start, FOLD3_BEAT_MS_MOBILE.align.len, FOLD3_BEAT_MS_MOBILE.type.start, FOLD3_BEAT_MS_MOBILE.type.len"
      },
      {
            "key": "legend",
            "label": "4 · Groups glide into the small legend at the side",
            "min": 100,
            "max": 4000,
            "step": 50,
            "value": 1250,
            "source": "js/groups.js — scales: FOLD4_GLIDE_MS_MOBILE, FOLD6_HEAD_UNTYPE_MS_MOBILE, FOLD6_MLEGEND_WIDTH_MS, FOLD6_MLEGEND_OPEN_MS, FOLD6_MFLY_HOLD_MS, FOLD6_MFLY_MS"
      },
      {
            "key": "squares",
            "label": "5 · The 8 coloured squares grow in",
            "min": 100,
            "max": 3000,
            "step": 10,
            "value": 550,
            "source": "js/groups.js — scales: FOLD5_SQUARES_MS_MOBILE"
      },
      {
            "key": "demo",
            "label": "6 · Hover demo: cursor glides, square swells, tooltip box opens (typing speed is 6b)",
            "min": 100,
            "max": 6000,
            "step": 50,
            "value": 1450,
            "source": "js/groups.js + js/fold8-tooltip.js — scales: FOLD7_CURSOR_MS, FOLD7_TOUCH_START_REST_MS, FOLD7_TOUCH_END_REST_MS, FOLD7_TOUCH_FADE_MS, FOLD7_TOUCH_POINTER_HOLD_MS, FOLD8_DEMO_GROW_MS, FOLD8_SQUARE_DIM_MS, FOLD8_TOOLTIP_DELAY_MS, FOLD8_GROW_MS"
      },
      {
            "key": "tipType",
            "label": "6b · Tooltip text typing speed — ms per letter, lower is faster",
            "min": 1,
            "max": 60,
            "step": 1,
            "value": 9,
            "source": "js/fold8-tooltip.js — scales: FOLD8_TYPE_MS_PER_CHAR_MOBILE"
      },
      {
            "key": "note",
            "label": "7 · Data-source note types into the legend",
            "min": 100,
            "max": 4000,
            "step": 50,
            "value": 1900,
            "source": "js/groups.js — scales: FOLD8_NOTE_MS"
      },
      {
            "key": "fly",
            "label": "8 · The 8 squares fly to their places on the timeline",
            "min": 100,
            "max": 4000,
            "step": 50,
            "value": 1500,
            "source": "js/groups.js — scales: FOLD9_FLY_MS, FOLD9_TOOLTIP_SHRINK_DELAY_MS, FOLD9_TOOLTIP_SHRINK_MS"
      },
      {
            "key": "axis",
            "label": "9 · The year line draws itself across the screen",
            "min": 100,
            "max": 6000,
            "step": 50,
            "value": 1750,
            "source": "page7.js — scales: P7_AXIS_INTRO_DURATION_MOBILE, P7_AXIS_INTRO_DOT_MS_MOBILE"
      },
      {
            "key": "month",
            "label": "10 · One month's dots pop onto the timeline as you scroll",
            "min": 100,
            "max": 4000,
            "step": 10,
            "value": 550,
            "source": "page7.js — scales: P7_ANIM_TOTAL_MS_MOBILE, P7_POP_MS_MOBILE"
      }
    ],
    ...
    apply: function (v) {
      var k;
      k = v.hero / 3114;
      if (typeof PAGE0_TITLE_MS_MOBILE !== 'undefined') PAGE0_TITLE_MS_MOBILE = Math.max(1, Math.round(1308 * k));
      if (typeof PAGE0_ROW_STAGGER_MS_MOBILE !== 'undefined') PAGE0_ROW_STAGGER_MS_MOBILE = Math.max(1, Math.round(31 * k));
      if (typeof PAGE0_POP_MS_MOBILE !== 'undefined') PAGE0_POP_MS_MOBILE = Math.max(1, Math.round(215 * k));
      if (typeof PAGE0_LOGO_FADE_MS_MOBILE !== 'undefined') PAGE0_LOGO_FADE_MS_MOBILE = Math.max(1, Math.round(692 * k));
      k = v.camps / 1600;
      if (typeof FOLD2_ENTRANCE_MS_MOBILE !== 'undefined') FOLD2_ENTRANCE_MS_MOBILE = Math.max(1, Math.round(1600 * k));
      k = v.groups / 1600;
      if (typeof FOLD3_BEAT_MS_MOBILE !== 'undefined') FOLD3_BEAT_MS_MOBILE.shrink.len = Math.max(1, Math.round(318 * k));
      if (typeof FOLD3_BEAT_MS_MOBILE !== 'undefined') FOLD3_BEAT_MS_MOBILE.align.start = Math.max(1, Math.round(119 * k));
      if (typeof FOLD3_BEAT_MS_MOBILE !== 'undefined') FOLD3_BEAT_MS_MOBILE.align.len = Math.max(1, Math.round(847 * k));
      if (typeof FOLD3_BEAT_MS_MOBILE !== 'undefined') FOLD3_BEAT_MS_MOBILE.type.start = Math.max(1, Math.round(635 * k));
      if (typeof FOLD3_BEAT_MS_MOBILE !== 'undefined') FOLD3_BEAT_MS_MOBILE.type.len = Math.max(1, Math.round(965 * k));
      k = v.legend / 1250;
      if (typeof FOLD4_GLIDE_MS_MOBILE !== 'undefined') FOLD4_GLIDE_MS_MOBILE = Math.max(1, Math.round(1250 * k));
      if (typeof FOLD6_HEAD_UNTYPE_MS_MOBILE !== 'undefined') FOLD6_HEAD_UNTYPE_MS_MOBILE = Math.max(1, Math.round(329 * k));
      if (typeof FOLD6_MLEGEND_WIDTH_MS !== 'undefined') FOLD6_MLEGEND_WIDTH_MS = Math.max(1, Math.round(112 * k));
      if (typeof FOLD6_MLEGEND_OPEN_MS !== 'undefined') FOLD6_MLEGEND_OPEN_MS = Math.max(1, Math.round(270 * k));
      if (typeof FOLD6_MFLY_HOLD_MS !== 'undefined') FOLD6_MFLY_HOLD_MS = Math.max(1, Math.round(263 * k));
      if (typeof FOLD6_MFLY_MS !== 'undefined') FOLD6_MFLY_MS = Math.max(1, Math.round(1250 * k));
      k = v.squares / 550;
      if (typeof FOLD5_SQUARES_MS_MOBILE !== 'undefined') FOLD5_SQUARES_MS_MOBILE = Math.max(1, Math.round(1000 * k));
      k = v.demo / 1450;
      if (typeof FOLD7_CURSOR_MS !== 'undefined') FOLD7_CURSOR_MS = Math.max(1, Math.round(370 * k));
      if (typeof FOLD7_TOUCH_START_REST_MS !== 'undefined') FOLD7_TOUCH_START_REST_MS = Math.max(1, Math.round(630 * k));
      if (typeof FOLD7_TOUCH_END_REST_MS !== 'undefined') FOLD7_TOUCH_END_REST_MS = Math.max(1, Math.round(800 * k));
      if (typeof FOLD7_TOUCH_FADE_MS !== 'undefined') FOLD7_TOUCH_FADE_MS = Math.max(1, Math.round(400 * k));
      if (typeof FOLD7_TOUCH_POINTER_HOLD_MS !== 'undefined') FOLD7_TOUCH_POINTER_HOLD_MS = Math.max(1, Math.round(1390 * k));
      if (typeof FOLD8_DEMO_GROW_MS !== 'undefined') FOLD8_DEMO_GROW_MS = Math.max(1, Math.round(350 * k));
      if (typeof FOLD8_SQUARE_DIM_MS !== 'undefined') FOLD8_SQUARE_DIM_MS = Math.max(1, Math.round(350 * k));
      if (typeof FOLD8_TOOLTIP_DELAY_MS !== 'undefined') FOLD8_TOOLTIP_DELAY_MS = Math.max(1, Math.round(250 * k));
      if (typeof FOLD8_GROW_MS !== 'undefined') FOLD8_GROW_MS = Math.max(1, Math.round(400 * k));
      k = v.tipType / 9;
      if (typeof FOLD8_TYPE_MS_PER_CHAR_MOBILE !== 'undefined') FOLD8_TYPE_MS_PER_CHAR_MOBILE = Math.max(1, Math.round(9 * k));
      k = v.note / 1900;
      if (typeof FOLD8_NOTE_MS !== 'undefined') FOLD8_NOTE_MS = Math.max(1, Math.round(1900 * k));
      k = v.fly / 1500;
      if (typeof FOLD9_FLY_MS !== 'undefined') FOLD9_FLY_MS = Math.max(1, Math.round(1500 * k));
      if (typeof FOLD9_TOOLTIP_SHRINK_DELAY_MS !== 'undefined') FOLD9_TOOLTIP_SHRINK_DELAY_MS = Math.max(1, Math.round(500 * k));
      if (typeof FOLD9_TOOLTIP_SHRINK_MS !== 'undefined') FOLD9_TOOLTIP_SHRINK_MS = Math.max(1, Math.round(400 * k));
      k = v.axis / 1750;
      if (typeof P7_AXIS_INTRO_DURATION_MOBILE !== 'undefined') P7_AXIS_INTRO_DURATION_MOBILE = Math.max(1, Math.round(1750 * k));
      if (typeof P7_AXIS_INTRO_DOT_MS_MOBILE !== 'undefined') P7_AXIS_INTRO_DOT_MS_MOBILE = Math.max(1, Math.round(300 * k));
      k = v.month / 550;
      if (typeof P7_ANIM_TOTAL_MS_MOBILE !== 'undefined') P7_ANIM_TOTAL_MS_MOBILE = Math.max(1, Math.round(550 * k));
      if (typeof P7_POP_MS_MOBILE !== 'undefined') P7_POP_MS_MOBILE = Math.max(1, Math.round(40 * k));
      if (typeof fold3BeatsRebuild === 'function') fold3BeatsRebuild();
    },

    /* EVERY open copy of the page follows, not just the one the panel tab
       picked as its host: with the page open in two places (editor browser +
       Chrome, or a stale tab) the sliders were moving a page nobody was
       watching. A second listener on the harness's own channel adopts any
       other page's state. Converges — it only sets what differs. */
    init: function (api) {
      if (typeof HBus !== 'function') return;
      var ch = HBus('harness:' + CONFIG.title);
      ch.onmessage = function (ev) {
        var m = ev.data || {};
        if (!m.inst || m.t !== 'state' || !m.V) return;
        CONFIG.sliders.forEach(function (s) {
          if (typeof m.V[s.key] === 'number' && m.V[s.key] !== api.get(s.key)) api.set(s.key, m.V[s.key]);
        });
      };
    },

    summary: function (v) {
      var parts = [["hero", "PAGE0_TITLE_MS_MOBILE", 1308, 3114], ["hero", "PAGE0_ROW_STAGGER_MS_MOBILE", 31, 3114], ["hero", "PAGE0_POP_MS_MOBILE", 215, 3114], ["hero", "PAGE0_LOGO_FADE_MS_MOBILE", 692, 3114], ["camps", "FOLD2_ENTRANCE_MS_MOBILE", 1600, 1600], ["groups", "FOLD3_BEAT_MS_MOBILE.shrink.len", 318, 1600], ["groups", "FOLD3_BEAT_MS_MOBILE.align.start", 119, 1600], ["groups", "FOLD3_BEAT_MS_MOBILE.align.len", 847, 1600], ["groups", "FOLD3_BEAT_MS_MOBILE.type.start", 635, 1600], ["groups", "FOLD3_BEAT_MS_MOBILE.type.len", 965, 1600], ["legend", "FOLD4_GLIDE_MS_MOBILE", 1250, 1250], ["legend", "FOLD6_HEAD_UNTYPE_MS_MOBILE", 329, 1250], ["legend", "FOLD6_MLEGEND_WIDTH_MS", 112, 1250], ["legend", "FOLD6_MLEGEND_OPEN_MS", 270, 1250], ["legend", "FOLD6_MFLY_HOLD_MS", 263, 1250], ["legend", "FOLD6_MFLY_MS", 1250, 1250], ["squares", "FOLD5_SQUARES_MS_MOBILE", 1000, 550], ["demo", "FOLD7_CURSOR_MS", 370, 1450], ["demo", "FOLD7_TOUCH_START_REST_MS", 630, 1450], ["demo", "FOLD7_TOUCH_END_REST_MS", 800, 1450], ["demo", "FOLD7_TOUCH_FADE_MS", 400, 1450], ["demo", "FOLD7_TOUCH_POINTER_HOLD_MS", 1390, 1450], ["demo", "FOLD8_DEMO_GROW_MS", 350, 1450], ["demo", "FOLD8_SQUARE_DIM_MS", 350, 1450], ["demo", "FOLD8_TOOLTIP_DELAY_MS", 250, 1450], ["demo", "FOLD8_GROW_MS", 400, 1450], ["tipType", "FOLD8_TYPE_MS_PER_CHAR_MOBILE", 9, 9], ["note", "FOLD8_NOTE_MS", 1900, 1900], ["fly", "FOLD9_FLY_MS", 1500, 1500], ["fly", "FOLD9_TOOLTIP_SHRINK_DELAY_MS", 500, 1500], ["fly", "FOLD9_TOOLTIP_SHRINK_MS", 400, 1500], ["axis", "P7_AXIS_INTRO_DURATION_MOBILE", 1750, 1750], ["axis", "P7_AXIS_INTRO_DOT_MS_MOBILE", 300, 1750], ["month", "P7_ANIM_TOTAL_MS_MOBILE", 550, 550], ["month", "P7_POP_MS_MOBILE", 40, 550]];
      var out = ['MOBILE — full animation lengths, and the constants they resolve to:'];
      CONFIG.sliders.forEach(function (s) {
        out.push('', s.label + ': ' + v[s.key] + 'ms' + (v[s.key] !== s.value ? ' (was ' + s.value + ')' : ' (unchanged)'));
        parts.forEach(function (p) {
          if (p[0] === s.key) out.push('    ' + p[1] + ' = ' + Math.max(1, Math.round(p[2] * v[s.key] / p[3])) + '  (was ' + p[2] + ')');
        });
      });
      return out.join('\n');
    }
  ...
  var API = {
    /* Push a live text block to the remote tab (CONFIG.readout). Custom UI
       never crosses the bus, so a harness that COLLECTS rather than tunes
       has no other way to show its result where the user is looking. */
    pushReadout: function () {
      if (!CONFIG.readout) return;
      try { post({ t: 'readout', text: CONFIG.readout() }); } catch (e) {}
    },
  ...
  function descriptor() {
    return {
      ...
      readout: CONFIG.readout ? { label: CONFIG.readoutLabel || null, text: CONFIG.readout() } : null,
