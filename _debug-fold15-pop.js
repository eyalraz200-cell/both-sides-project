...
(function () {
  ...
  var CONFIG = {
    title: 'fold15-pop',
    label: 'how fast the new dots pop in',
    fold: '@fold15',
    viewport: 'desktop',
    remoteOnly: true,
    goTo: '#page-14',
    goLabel: '@fold15',
    sliders: [
      { key: 'pop', label: 'how long the new dots take to pop in (ms)', min: 150, max: 1200, step: 5,
        value: 775, ship: 775, source: 'js/fold11.js FOLD14_POP_MS' }
    ],
    apply: function (v) {
      if (typeof FOLD14_POP_MS === 'undefined') return;
      /* The trigger resolves its duration per frame off fold14TotalMs(), so the
         new value plays on the NEXT crossing — scroll up past @fold15's card and
         back down to replay it (Go lands you on the fold; nudge up, then down). */
      FOLD14_POP_MS = v.pop;
    },
    summary: function (v) { return 'FOLD14_POP_MS = ' + v.pop + ' (fly stays ' + (typeof FOLD14_FLY_MS !== 'undefined' ? FOLD14_FLY_MS : '?') + ')'; }
  };
  // ------------------------------------------------------------ END CONFIG --
