...
(function () {
  // ---------------------------------------------------------------- CONFIG --
  var CONFIG = {
    title: 'fold7-timeline',
    label: 'fold 7 demo: the phases',
    fold: '@fold7',
    viewport: 'mobile',
    remoteOnly: true,
    ...
    /* Every beat of the mobile @fold7 demo, in ms. The timeline above the
       sliders draws them on one clock: drag a segment's right edge or type a
       number. To replay: Go lands just above the crossing — scroll down a
       little (and back up to rewind). */
    sliders: [
      { key: 'fadeIn',      label: 'glass fades in',                       min: 0, max: 2000, step: 10, value: 400,  group: 'glass', source: 'js/groups.js — FOLD7_TOUCH_FADE_IN_MS' },
      { key: 'startRest',   label: 'glass rests at its start',             min: 0, max: 3000, step: 10, value: 780,  group: 'glass', source: 'js/groups.js — FOLD7_TOUCH_START_REST_MS' },
      { key: 'cursorMs',    label: 'glass glides to the square',           min: 50, max: 3000, step: 10, value: 370, group: 'glass', source: 'js/groups.js — FOLD7_CURSOR_MS' },
      { key: 'arriveFrac',  label: 'where in the glide the square reacts (0–1)', min: 0, max: 1, step: 0.05, value: 0.5, group: 'glass', source: 'js/groups.js — FOLD7_CURSOR_ARRIVE_FRAC' },
      { key: 'endRest',     label: 'glass rests on the square',            min: 0, max: 4000, step: 10, value: 800,  group: 'glass', source: 'js/groups.js — FOLD7_TOUCH_END_REST_MS' },
      { key: 'fadeMs',      label: 'glass fades away',                     min: 0, max: 2000, step: 10, value: 400,  group: 'glass', source: 'js/groups.js — FOLD7_TOUCH_FADE_MS' },
      { key: 'pointerHold', label: 'hand stays before fading',             min: 0, max: 4000, step: 10, value: 1390, group: 'hand', source: 'js/groups.js — FOLD7_TOUCH_POINTER_HOLD_MS' },
      { key: 'pointerOut',  label: 'hand fades',                           min: 0, max: 2000, step: 10, value: 300,  group: 'hand', source: 'js/groups.js — FOLD7_TOUCH_POINTER_OUT_MS' },
      { key: 'growDelay',   label: 'square waits after the arrival',       min: 0, max: 2000, step: 10, value: 0,    group: 'square', source: 'js/groups.js — FOLD8_DEMO_GROW_DELAY_MS' },
      { key: 'growMs',      label: 'square swells',                        min: 50, max: 2000, step: 10, value: 350, group: 'square', source: 'js/groups.js — FOLD8_DEMO_GROW_MS' },
      { key: 'dimMs',       label: 'the other squares dim',                min: 50, max: 2000, step: 10, value: 350, group: 'square', source: 'js/groups.js — FOLD8_SQUARE_DIM_MS' },
      { key: 'tipDelay',    label: 'tooltip waits after the arrival',      min: 0, max: 2000, step: 10, value: 250,  group: 'tooltip', source: 'js/fold8-tooltip.js — FOLD8_TOOLTIP_DELAY_MS' },
      { key: 'tipGrow',     label: 'tooltip frame grows',                  min: 50, max: 2000, step: 10, value: 400, group: 'tooltip', source: 'js/fold8-tooltip.js — FOLD8_GROW_MS' },
      { key: 'typeMs',      label: 'typing, per character',                min: 1, max: 80, step: 1, value: 15,      group: 'tooltip', source: 'js/fold8-tooltip.js — FOLD8_TYPE_MS_PER_CHAR' },
    ],
    colors: [],

    timeline: {
      label: 'The demo on one clock (arrive = rest + glide × the reaction point)',
      lanes: [
        { label: 'glass in', segs: [{ key: 'fadeIn', label: 'fade in' }] },
        { label: 'glass',
          segs: [{ key: 'startRest', label: 'rest' }, { key: 'cursorMs', label: 'glide' },
                 { key: 'endRest', label: 'rest on it' }, { key: 'fadeMs', label: 'fade' }],
          marks: [{ label: 'arrive', at: [{ key: 'startRest' }, { key: 'cursorMs', mulKey: 'arriveFrac' }] }] },
        { label: 'hand', segs: [{ key: 'pointerHold', label: 'shown' }, { key: 'pointerOut', label: 'fade' }] },
        { label: 'square', offset: [{ key: 'startRest' }, { key: 'cursorMs', mulKey: 'arriveFrac' }],
          segs: [{ key: 'growDelay', label: 'wait' }, { key: 'growMs', label: 'swell' }] },
        { label: 'others', offset: [{ key: 'startRest' }, { key: 'cursorMs', mulKey: 'arriveFrac' }],
          segs: [{ key: 'dimMs', label: 'dim' }] },
        { label: 'tooltip', offset: [{ key: 'startRest' }, { key: 'cursorMs', mulKey: 'arriveFrac' }],
          segs: [{ key: 'tipDelay', label: 'wait' }, { key: 'tipGrow', label: 'grow' },
                 { key: 'typeMs', label: 'typing (96 chars)', mul: 96 }] },
      ]
    },

    apply: function (v) {
      FOLD7_TOUCH_FADE_IN_MS      = v.fadeIn;
      FOLD7_TOUCH_START_REST_MS   = v.startRest;
      FOLD7_CURSOR_MS             = v.cursorMs;
      FOLD7_CURSOR_ARRIVE_FRAC    = v.arriveFrac;
      FOLD7_TOUCH_END_REST_MS     = v.endRest;
      FOLD7_TOUCH_FADE_MS         = v.fadeMs;
      FOLD7_TOUCH_POINTER_HOLD_MS = v.pointerHold;
      FOLD7_TOUCH_POINTER_OUT_MS  = v.pointerOut;
      FOLD8_DEMO_GROW_DELAY_MS    = v.growDelay;
      FOLD8_DEMO_GROW_MS          = v.growMs;
      FOLD8_SQUARE_DIM_MS         = v.dimMs;
      FOLD8_TOOLTIP_DELAY_MS      = v.tipDelay;
      FOLD8_GROW_MS               = v.tipGrow;
      FOLD8_TYPE_MS_PER_CHAR      = v.typeMs;
      if (typeof updateGroups === 'function') updateGroups();
    },

    init: null, custom: null,
    goTo: '#page-6', goLabel: '@fold7',
    /* Land just ABOVE the crossing, with the demo rewound, so one short scroll
       down plays it with the current numbers. */
    onGo: function () {
    ...
    width: 560, collapseSliders: true, knobCols: 2,
    summary: function (v) {
      return [
        'var FOLD7_TOUCH_FADE_IN_MS = ' + v.fadeIn + ';',
        'var FOLD7_TOUCH_START_REST_MS = ' + v.startRest + ';',
        'var FOLD7_CURSOR_MS = ' + v.cursorMs + ';',
        'var FOLD7_CURSOR_ARRIVE_FRAC = ' + v.arriveFrac + ';',
        'var FOLD7_TOUCH_END_REST_MS = ' + v.endRest + ';',
        'var FOLD7_TOUCH_FADE_MS = ' + v.fadeMs + ';',
        'var FOLD7_TOUCH_POINTER_HOLD_MS = ' + v.pointerHold + ';',
        'var FOLD7_TOUCH_POINTER_OUT_MS = ' + v.pointerOut + ';',
        'var FOLD8_DEMO_GROW_DELAY_MS = ' + v.growDelay + ';',
        'var FOLD8_DEMO_GROW_MS = ' + v.growMs + ';',
        'var FOLD8_SQUARE_DIM_MS = ' + v.dimMs + ';',
        'var FOLD8_TOOLTIP_DELAY_MS = ' + v.tipDelay + ';',
        'var FOLD8_GROW_MS = ' + v.tipGrow + ';',
        'var FOLD8_TYPE_MS_PER_CHAR = ' + v.typeMs + ';',
      ].join('\n');
    }
  };
  // ------------------------------------------------------------ END CONFIG --
