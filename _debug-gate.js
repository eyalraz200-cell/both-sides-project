...
(function () {
  ...
  var CONFIG = {
    title: 'gate',
    label: 'the work-in-progress notice: look, and how dark the page goes',
    fold: '@fold1',
    viewport: 'both',
    remoteOnly: true,
    tabs: [], tab: null, onTab: null,

    /* The four candidate looks. Press Go to re-open the gate (it also clears
       the "already seen" flag), then 1–4 to flip between them in place. */
    modes: [
      { key: '1', id: 'doc', label: 'dashed title card (the piece’s own)' },
      { key: '2', id: 'dark', label: 'inverted dark card' },
      { key: '3', id: 'squares', label: 'light card, the six colours grow in' },
      { key: '4', id: 'min', label: 'no card — text on the dark' },
    ],
    mode: 'doc',
    toggles: [],

    /* The darkening itself — the thing being compared. Each style ships its own
       default (a card can carry a lighter page behind it than bare text can),
       so the two knobs start wherever the picked style left them. */
    sliders: [
      { key: 'dark', label: 'how dark the page goes behind it', min: 0, max: 0.9, step: 0.01, value: 0.45,
        source: 'style.css — .shk-gate --gate-dark' },
      { key: 'blur', label: 'how blurred the page goes behind it', min: 0, max: 14, step: 0.5, value: 2,
        source: 'style.css — .shk-gate --gate-blur' },
      { key: 'fade', label: 'how long the notice takes to clear (ms)', min: 120, max: 1200, step: 20, value: 420,
        source: 'style.css — .shk-gate transition + SHK_GATE_FADE_MS, js/intro-gate.js' },
    ],
    colors: [],

    apply: function (v, mode) {
      var g = document.getElementById('shkGate');
      if (g) g.setAttribute('data-gate-style', mode || 'doc');
      /* Written as a stylesheet rather than inline styles so the per-style
         defaults in style.css still lose to it at the same specificity, and so
         one Reset wipes every override at once. */
      CONFIG.style.textContent =
        '.shk-gate[data-gate-style] {' +
        '  --gate-dark: ' + v.dark + ';' +
        '  --gate-blur: ' + v.blur + 'px;' +
        '  transition-duration: ' + v.fade + 'ms;' +
        '}';
    },

    /* Picking a style snaps the two knobs to THAT style's shipped defaults,
       so each candidate is judged as it would actually ship rather than
       inheriting the previous one's tuning. */
    init: function (api) {
      var DEFAULTS = { doc: 0.45, dark: 0.6, squares: 0.45, min: 0.72 };
      var last = null;
      var tick = function () {
        var m = document.getElementById('shkGate');
        m = m && m.getAttribute('data-gate-style');
        if (m && m !== last) { last = m; if (DEFAULTS[m] != null) { api.set('dark', DEFAULTS[m]); api.refresh(); } }
        requestAnimationFrame(tick);
      };
      requestAnimationFrame(tick);
    },

    custom: null, timeline: null,

    /* The gate is dismissed the moment you press its button, and it is
       once-per-browser — so Go is what brings it back, with the flag cleared
       so a plain refresh shows it too. Nothing scrolls: the gate is fixed and
       @fold1 is already at the top. */
    goTo: '#page-0', goLabel: 'show the notice',
    onGo: function () { if (typeof shkGateReopen === 'function') shkGateReopen(); },

    width: 0, collapseSliders: false, knobCols: 1,
    summary: function (v, mode) {
      return 'style      ' + (mode || 'doc') + '   (data-gate-style in project.html)\n' +
             '--gate-dark  ' + v.dark + '\n' +
             '--gate-blur  ' + v.blur + 'px\n' +
             'fade         ' + v.fade + 'ms  (style.css transition + SHK_GATE_FADE_MS)';
    }
  };
  // ------------------------------------------------------------ END CONFIG --
