/* SCAFFOLDING — @fold1 hero title/subtitle alignment, restored from 41dcce9
   onto the current harness template. Never ships. */
(function () {
  var l = document.createElement('link'); l.rel = 'stylesheet';
  l.href = 'https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Hebrew:wght@100;200;300;400;500;600;700&display=swap';
  ...
  var CONFIG = {
    ...
    viewport: 'both',
    remoteOnly: true,
    goTo: '#page-0', goLabel: '@fold1', onGo: null,
    ...
    sliders: [
      { key: 'size', label: 'text size', min: 10, max: 72, step: 0.5, value: 42,
        source: 'style.css:2265 .page0-title / :2292 .page0-subtitle — the `font:` shorthand' },
      { key: 'lh', label: 'space between lines', min: 0.8, max: 2.2, step: 0.01, value: 1.31,
        source: 'style.css — neither rule sets one today (browser `normal`); a bake adds line-height' },
      { key: 'gap', label: 'gap from last line to the dots', min: -20, max: 80, step: 0.5, value: 23.7,
        source: 'style.css:2253 .page0-title top / :2290 .page0-subtitle top — bakes back as calc(50% - Npx)' },
      { key: 'width', label: 'text box width', min: 60, max: 400, step: 1, value: 185,
        source: 'style.css .page0-title width (185px; mobile min(185px, 50vw - 20px)) / .page0-subtitle width (125px)' },
      { key: 'weight', label: 'how heavy the letters are', min: 100, max: 700, step: 100, value: 300,
        source: 'style.css .page0-title font (300, IBM Plex Sans Hebrew) / .page0-subtitle font (300, IBM Plex Sans Hebrew)' },
      { key: 'nudge', label: 'nudge sideways (letters vs. dot edge)', min: -12, max: 12, step: 0.5, value: 0,
        source: 'added to .page0-title / .page0-subtitle left — letters carry side bearing, the box edge is not the ink edge' },
      { key: 'colGap', global: true, label: 'gap between the two dot columns', min: 0, max: 80, step: 1, value: 20,
        source: 'page1.js:28-29 PAGE0_DOT_COLS centerX/offsetX ±13.5px (gap = 2×13.5 − 7px square = 20)' },
    ],
    ...
    colors: [],
    // compare/: which edge of its dot column each text lines up with. The
    // right-hand column spans 50%+10…+17px, the left one 50%-17…-10px
    // (page1.js PAGE0_DOT_COLS, 7px squares). '0' = as shipped: title box at
    // +8px (2px past the column's left edge), subtitle box ending at -10px.
    modes: [
      { key: '1', id: 't-left',  group: 'title',    label: 'title: its dots\u2019 left edge' },
      { key: '2', id: 't-right', group: 'title',    label: 'title: its dots\u2019 right edge' },
      { key: '3', id: 's-right', group: 'subtitle', label: 'subtitle: its dots\u2019 right edge' },
      { key: 'a', id: 'o1628', group: 'preset', label: 'option 1 (16:28): weight 300/300, sub lines 1.61' },
      { key: 'b', id: 'o1632', group: 'preset', label: 'option 2 (16:32): weight 200/200, sub lines 1.61' },
      { key: 'c', id: 'o1635', group: 'preset', label: 'option 3 (16:35): weight 200/200, sub lines 1.63' },
      // Weight pairings: pick the title's and the subtitle's weight separately,
      // on top of any preset or the live knobs. '0' in a group = use the knob.
      { key: '5', id: 'tw200', group: 'title weight',    label: 'title 200' },
      { key: '6', id: 'tw300', group: 'title weight',    label: 'title 300' },
      { key: '7', id: 'tw400', group: 'title weight',    label: 'title 400' },
      { key: '8', id: 'sw200', group: 'subtitle weight', label: 'subtitle 200' },
      { key: '9', id: 'sw300', group: 'subtitle weight', label: 'subtitle 300' },
      { key: 'w', id: 'sw400', group: 'subtitle weight', label: 'subtitle 400' },
      { key: 'h', id: 'hadassah', group: 'preset', label: 'the old hero: Hadassah title + Assistant subtitle' },
      { key: 's', id: 'shipped',  group: 'preset', label: 'Plex before the 16:43 bake' },
      { key: '4', id: 's-left',  group: 'subtitle', label: 'subtitle: its dots\u2019 left edge' },
    ],
    mode: null,
    ...
    toggles: [
      { id: 'rulers', label: 'rulers (all of them)', key: 'R', on: true },
      { id: 'noBr', label: 'subtitle: ignore the forced line breaks (width decides)', key: 'B', on: false },
    ],
    ...
    apply: function (v, mode, tab, on) {
      var m = mode || {};
      // compare/ presets: a frozen snapshot REPLACES the live knobs while
      // picked; '0' on the preset group hands back the live knobs untouched.
      var PRESETS = {
        shipped:  { g: 20, t: null, s: null, noBr: false,
          title:    { size: 42, lh: 1.31, gap: 23.73, width: 185, weight: v.title.weight, nudge: 0 },
          subtitle: { size: 18, lh: 1.52, gap: 20, width: 125, weight: v.subtitle.weight, nudge: 0 } },
      };
      // Your three Copies, exactly as sent (16:28, 16:32, 16:35).
      function opt(tw, sw, slh) {
        return { g: 20, t: 'keep', s: 's-right', noBr: true,
          title:    { size: 44, lh: 1.17, gap: 22, width: 172, weight: tw, nudge: -1.5 },
          subtitle: { size: 16, lh: slh,  gap: 20, width: 106, weight: sw, nudge: 2 } };
      }
      PRESETS.o1628 = opt(300, 300, 1.61);
      PRESETS.o1632 = opt(200, 200, 1.61);
      PRESETS.o1635 = opt(200, 200, 1.63);
      // The hero before the Plex swap (commit 916361c): Hadassah 400 title,
      // Assistant 300 subtitle, original sizes/leading/gaps/widths, <br>s on.
      PRESETS.hadassah = { g: 20, t: null, s: null, noBr: false,
        title:    { size: 42, lh: 1.31, gap: 23.73, width: 185, weight: 400, nudge: 0, family: "'HadassahFriedlaender', serif" },
        subtitle: { size: 18, lh: 1.52, gap: 20, width: 125, weight: 300, nudge: 0, family: "'Assistant', sans-serif" } };
      var P = PRESETS[m.preset];
      var TW = m['title weight'], SW = m['subtitle weight'];
      if (P) {
        v = { __global: { colGap: P.g }, title: P.title, subtitle: P.subtitle };
        m = { title: P.t === 'keep' ? null : P.t, subtitle: P.s };
        on = Object.assign({}, on, { noBr: P.noBr });
      }
      if (TW || SW) {
        v = { __global: v.__global, title: Object.assign({}, v.title), subtitle: Object.assign({}, v.subtitle) };
        if (TW) v.title.weight = +TW.slice(2);
        if (SW) v.subtitle.weight = +SW.slice(2);
      }
      // Dot columns: move them, rebuilding the dots exactly as a resize does
      // (js/bootstrap.js), only when the gap actually changed.
      var gap = (v.__global || v).colGap, half = gap / 2, c = half + PAGE0_DOT_SQ / 2;
      if (window.__heroColGap == null) window.__heroColGap = 20;   // shipped — no rebuild on load
      if (window.__heroColGap !== gap && typeof buildPage0AllDots === 'function') {
        window.__heroColGap = gap;
        PAGE0_DOT_COLS[0].centerX = 'calc(50% + ' + c + 'px)'; PAGE0_DOT_COLS[0].offsetX = c;
        PAGE0_DOT_COLS[1].centerX = 'calc(50% - ' + c + 'px)'; PAGE0_DOT_COLS[1].offsetX = -c;
        buildPage0AllDots(); assignFold2Fillers();
        if (page0EntranceDone) PAGE0_DECORATIVE_DOT_ELS.forEach(function (d) {
          d.el.style.opacity = '1'; d.el.style.transform = 'scale(1)'; d.popped = true;
        });
        layoutGroups(); draw();
      }
      // Text follows the columns. Right column spans +half…+half+7, left one
      // −half−7…−half. Shipped: title box 2px left of its column's left edge.
      var tX = { 't-left': half, 't-right': half + PAGE0_DOT_SQ }[m.title];
      if (tX == null) tX = half - 2;
      var sX = m.subtitle === 's-left' ? -(half + PAGE0_DOT_SQ) : -half;
      tX += v.title.nudge; sX += v.subtitle.nudge;
      window.__heroX = { t: tX, s: sX, half: half };
      var tEl = document.querySelector('.page0-title'), sEl = document.querySelector('.page0-subtitle');
      if (tEl) tEl.style.left = 'calc(50% + ' + tX + 'px)';
      if (sEl) sEl.style.left = 'calc(50% - ' + (-sX) + 'px)';
      var SUBS = {
        title:    { sel: '.page0-title',    startOffsetY: 0 },
        subtitle: { sel: '.page0-subtitle', startOffsetY: 2 * PAGE0_DOT_STEP },
      };
      // Swap each <br> for a real space (and back) — hiding the <br> alone
      // glued the neighbouring words together.
      if (on && on.noBr) document.querySelectorAll('.page0-subtitle br').forEach(function (b) {
        var sp = document.createElement('span'); sp.className = 'hero-br-space'; sp.textContent = ' ';
        b.replaceWith(sp);
      });
      else document.querySelectorAll('.page0-subtitle .hero-br-space').forEach(function (sp) {
        sp.replaceWith(document.createElement('br'));
      });
      Object.keys(SUBS).forEach(function (id) {
        ...
        el.style.fontFamily = vals.family || '';
        el.style.fontWeight = vals.weight;
      ...
      window.__heroRulersOn = on ? { rTitle: on.rulers, rSubtitle: on.rulers, rTops: on.rulers, rCols: on.rulers } : null;
    ...
    init: function (api) {
      ...
      /* Height of a Hebrew letter's ink above the baseline (ה has no
         ascender/descender) — the "top of the line" the eye reads. */
      window.__heroLetterTop = function (el) {
        var cs = getComputedStyle(el);
        mctx.font = cs.fontStyle + ' ' + cs.fontWeight + ' ' + cs.fontSize + ' ' + cs.fontFamily;
        return mctx.measureText('\u05D4').actualBoundingBoxAscent || parseFloat(cs.fontSize) * 0.55;
      };
      ...
      window.__heroSyncRulers = function () {
        ...
        [['.page0-title', '#e0245e', 'rTitle'], ['.page0-subtitle', '#1f8fff', 'rSubtitle']]
          .forEach(function (row) {
            ...
            var top = on.rTops === false ? 0 : window.__heroLetterTop(el);
            window.__heroBaselines(el).forEach(function (y, i) {
              rows.push({ y: y, color: row[1], label: row[0].slice(6) + ' L' + (i + 1) });
              if (top) rows.push({ y: y - top, color: row[1], label: row[0].slice(6) + ' L' + (i + 1) + ' top', dotted: true });
            });
          });
        ...
        pool.forEach(function (d, i) {
          ...
          d.style.borderTopStyle = rows[i].dotted ? 'dotted' : 'dashed';
        });
      };

      // Column-edge guides: short vertical ticks beside each text block, so
      // letter-to-dot alignment is seen, not guessed.
      var guides = [0, 1, 2, 3].map(function () {
        var g = document.createElement('div');
        g.style.cssText = 'position:absolute;width:0;border-left:1px dashed #0a0;opacity:.8';
        layer.appendChild(g); return g;
      });
      function syncGuides() {
        var on = window.__heroRulersOn || {}, x = window.__heroX, cx = window.innerWidth / 2;
        var t = document.querySelector('.page0-title'), st = document.querySelector('.page0-subtitle');
        if (!x || !t || !st) return;
        var tr = t.getBoundingClientRect(), sr = st.getBoundingClientRect();
        var h = x.half, q = PAGE0_DOT_SQ;
        [[cx + h, tr, on.rTitle], [cx + h + q, tr, on.rTitle], [cx - h, sr, on.rSubtitle], [cx - h - q, sr, on.rSubtitle]]
          .forEach(function (d, i) {
            var g = guides[i];
            var full = on.rCols !== false;
            g.style.display = (on.rulers === false || on.rCols === false) ? 'none' : '';
            g.style.left = d[0] + 'px';
            g.style.top = full ? '0px' : d[1].top + 'px';
            g.style.height = full ? '100%' : d[1].height + 'px';
          });
      }
      (function tick() { window.__heroSyncRulers(); syncGuides(); requestAnimationFrame(tick); })();
    },
      ...
      [['title', '.page0-title', 0], ['subtitle', '.page0-subtitle', 2 * PAGE0_DOT_STEP]]
        .forEach(function (row) {
          ...
          out.push(row[1] + ' { font-size: ' + v[row[0]].size + 'px; line-height: '
                 + v[row[0]].lh + '; font-weight: ' + v[row[0]].weight + '; width: ' + v[row[0]].width + 'px; top: calc(50% - ' + above + 'px); }'
                 + '   /* gap ' + v[row[0]].gap + 'px, measured at '
                 + window.innerWidth + '×' + window.innerHeight + ', '
                 + (window.__heroLineCount ? window.__heroLineCount(row[1]) : '?') + ' lines */'
                 + (row[0] === 'subtitle' && document.querySelector('.page0-subtitle .hero-br-space') ? '\n  (forced <br> breaks OFF — replace them with spaces in index.html:141)' : ''));
        });
      return out.join('\n');
    }
  };
