...
(function () {
  ...
  var CONFIG = {
    ...
    sliders: [
      { key: 'zoomOut', label: 'how far the glass pulls back on a big dot', min: 0, max: 1, step: 0.05,
        value: 0.5, ship: 0.5, source: 'page7.js P7_LOUPE_ZOOM_OUT' },
      { key: 'topY', label: 'tooltip when it sits at the top', min: 0, max: 400, step: 2,
        value: 96, ship: 96, source: 'js/fold8-tooltip.js P7_TIP_TOP_PX (its TOP edge)' },
      { key: 'botY', label: 'tooltip when it sits at the bottom', min: 0, max: 300, step: 2,
        value: 8, ship: 8, source: 'js/fold8-tooltip.js P7_TIP_BOTTOM_PX (its BOTTOM edge, up from the screen bottom)' },
      { key: 'switchY', label: 'the line the glass crosses to send it to the bottom', min: 0, max: 852, step: 4,
        value: 300, ship: 300, source: 'js/fold8-tooltip.js P7_TIP_SWITCH_Y' }
    ],

    toggles: [
      { key: 'm', id: 'marks', label: 'show the switch line', on: true }
    ],

    apply: function (v, mode, tab, on) {
      if (typeof P7_TIP_SWITCH_Y === 'undefined') return;
      P7_LOUPE_ZOOM_OUT = v.zoomOut;
      P7_TIP_TOP_PX     = v.topY;
      P7_TIP_BOTTOM_PX  = v.botY;
      P7_TIP_SWITCH_Y   = v.switchY;
      showMarks(!!on.marks, v.switchY);
      /* Re-place the frame at once, so dragging a slider moves it live rather
         than on the next redraw. */
      var el = document.getElementById('page9Tooltip');
      if (el && typeof tooltipDockMobile === 'function') tooltipDockMobile(el);
      if (typeof draw === 'function') draw();
    },

    summary: function (v) {
      return 'page7.js: P7_LOUPE_ZOOM_OUT = ' + v.zoomOut +
             '\njs/fold8-tooltip.js: P7_TIP_TOP_PX = ' + v.topY +
             '   P7_TIP_BOTTOM_PX = ' + v.botY +
             '   P7_TIP_SWITCH_Y = ' + v.switchY;
    }
  };

  /* The switch line is a TRIGGER, so it gets a visible marker — a short tick in
     the LEFT MARGIN at the threshold plus a live one for the glass's top edge
     being tested against it, never a rule across the artwork. Grey = armed,
     orange = the glass is past it (the frame is at the bottom). */
  var markEl = null, markTick = null, markLive = null, markRaf = 0;
  function showMarks(on, y) {
    if (!on) { if (markEl) markEl.style.display = 'none'; cancelAnimationFrame(markRaf); markRaf = 0; return; }
    if (!markEl) {
      markEl = document.createElement('div');
      markEl.style.cssText = 'position:fixed;left:0;top:0;width:64px;height:100%;' +
        'pointer-events:none;z-index:2147483000;font:9px ui-monospace,monospace';
      markTick = document.createElement('div');
      markTick.style.cssText = 'position:absolute;left:0;width:26px;border-top:1px solid #888;' +
        'color:#888;padding-left:28px;white-space:nowrap;line-height:0';
      markLive = document.createElement('div');
      markLive.style.cssText = 'position:absolute;left:4px;width:14px;border-top:2px solid #0a7;line-height:0';
      markEl.appendChild(markTick); markEl.appendChild(markLive);
      document.body.appendChild(markEl);
    }
    markEl.style.display = '';
    markTick.style.top = y + 'px';
    markTick.textContent = 'tooltip flips here';
    if (!markRaf) tick();
  }
  function tick() {
    markRaf = requestAnimationFrame(tick);
    if (!markEl || markEl.style.display === 'none') return;
    var live = (typeof p7Inspect !== 'undefined' && p7Inspect.dragging &&
                typeof window.__loupeTop === 'number') ? window.__loupeTop : null;
    markLive.style.display = live === null ? 'none' : '';
    if (live !== null) {
      markLive.style.top = live + 'px';
      var past = typeof p7TipAtBottom !== 'undefined' && p7TipAtBottom;
      markLive.style.borderTopColor = past ? '#e07b00' : '#0a7';
      markTick.style.borderTopColor = markTick.style.color = past ? '#e07b00' : '#888';
    }
  }
  // ------------------------------------------------------------ END CONFIG --
