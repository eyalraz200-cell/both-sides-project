...
(function () {
  ...
  var CONFIG = {
    title: 'misclassified',
    label: 'flag wrongly-classified dots',
    fold: '@fold13',
    /* @fold13's hover hit-test is desktop-only (p9HoverInit bails on mobile),
       and this harness rides on it — with no hover there is nothing to click. */
    viewport: 'desktop',
    /* The flagged list now travels to the panel tab as a readout, so this can
       be a normal remote-first harness — no chrome sitting on the fold being
       inspected, which matters here because the thing being clicked IS the
       artwork. Press H on the page if the tab is not open. */
    remoteOnly: true,
    readoutLabel: 'flagged dots',
    tabs: [], tab: null, onTab: null,
    modes: [], mode: null, toggles: [],

    sliders: [
      { key: 'grow', label: 'how much bigger a flagged dot draws', min: 0, max: 10, step: 1, value: 2,
        source: 'this harness only — 0 paints the dot at its true size; the dots are ~2px here, so a little helps' }
    ],
    colors: [
      { key: 'tint', label: 'flagged dot colour', value: '#FF0000',
        source: 'this harness only' }
    ],

    /* Nothing in the page changes. This harness only READS p9.hoveredEvent and
       p9.lastPositions and paints its own overlay — page9.js is untouched, and
       the FINALIZED state-1 drop animation is never in the call path. */
    apply: function () { if (CONFIG._paint) CONFIG._paint(); },

    init: function (api) {
      CONFIG._api = api;
      /* ON DISK, not in localStorage. This list is research gathered over
         sessions — a cleared cache, a different browser or a phone must not
         lose it, and on disk Claude can read it directly instead of the user
         pasting a hundred row ids. localStorage stays as a same-tab mirror so
         a dead server degrades to the old behaviour rather than losing a click. */
      var LS = 'harness:misclassified:list';
      CONFIG._flagged = [];
      try { CONFIG._flagged = JSON.parse(localStorage.getItem(LS)) || []; } catch (e) {}

      var save = function () {
        try { localStorage.setItem(LS, JSON.stringify(CONFIG._flagged)); } catch (e) {}
        try {
          fetch('/__flags__', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ rows: CONFIG._flagged })
          }).catch(function () {});
        } catch (e) {}
        if (CONFIG._api) CONFIG._api.pushReadout();
      };
      CONFIG._save = save;

      /* The file wins on load: it is what survived the refresh. */
      fetch('/__flags__').then(function (r) { return r.json(); }).then(function (d) {
        if (d && Array.isArray(d.rows)) {
          CONFIG._flagged = d.rows;
          try { localStorage.setItem(LS, JSON.stringify(d.rows)); } catch (e) {}
          if (CONFIG._sync) CONFIG._sync();
          if (CONFIG._paint) CONFIG._paint();
          if (CONFIG._api) CONFIG._api.pushReadout();
        }
      }).catch(function () {});

      /* ---- the marker overlay -------------------------------------------
         A direct child of .layout, not of .graphic-col: that column makes its
         own stacking context and traps any z-index put inside it. */
      var layer = document.createElement('div');
      layer.style.cssText = 'position:fixed;inset:0;pointer-events:none;z-index:9000';
      (document.querySelector('.layout') || document.body).appendChild(layer);

      function onFold13() {
        return typeof currentPage !== 'undefined' && currentPage === 12
          && typeof p9 !== 'undefined' && p9.lastPositions;
      }

      /* Positions are rebuilt every frame by p9PlaceDot, so the rings have to
         be re-read every frame too — a flagged dot moves with its column. */
      function paint() {
        if (!onFold13()) { layer.innerHTML = ''; return; }
        var v = CONFIG._api ? CONFIG._api.values() : { grow: 2, tint: '#FF0000' };
        var rect = canvas.getBoundingClientRect();
        var byId = {};
        CONFIG._flagged.forEach(function (f) { byId[f.rowId] = true; });
        var html = '';
        /* A filled SQUARE painted exactly over the dot, at the dot's own size —
           so the dot simply reads as red, rather than wearing a ring. Square,
           because every dot in this project is a square. It is an overlay, not
           a recolour: page9.js's draw path is never touched. */
        p9.lastPositions.forEach(function (pos, ev) {
          if (!byId[ev.rowId]) return;
          var s = (pos.sq || 2) + v.grow;
          var cx = rect.left + pos.x + (pos.sq || 0) / 2;
          var cy = rect.top + pos.y + (pos.sq || 0) / 2;
          html += '<div style="position:absolute;left:' + (cx - s / 2) + 'px;top:'
               + (cy - s / 2) + 'px;width:' + s + 'px;height:' + s + 'px;'
               + 'background:' + v.tint + '"></div>';
        });
      ...
      /* ---- the click ------------------------------------------------------
         Reads the dot the page's OWN hover scan already resolved one pointer-
         move earlier, rather than re-implementing the hit test. Guarded the
         same way that scan is, so a drag is never mistaken for a flag. */
      /* Arrow keys move the selection (that part is page9.js and ships); Enter
         or Space flags whatever is selected, which is this harness's job and
         does not. Without it, keyboard traversal would still need a mouse
         click to mark anything, which defeats the point. */
      window.addEventListener('keydown', function (e) {
        if (e.key !== 'Enter' && e.key !== ' ') return;
        if (!onFold13() || p9.anim || !p9.hoveredEvent) return;
        var t = e.target;
        if (t && (t.isContentEditable || /^(INPUT|TEXTAREA|SELECT)$/.test(t.tagName || '')
            || (t.closest && t.closest('.page9-pill, .hp')))) return;
        e.preventDefault();
        toggle(p9.hoveredEvent);
      });

      function toggle(ev) {
        var at = -1;
        CONFIG._flagged.forEach(function (f, i) { if (f.rowId === ev.rowId) at = i; });
        if (at >= 0) CONFIG._flagged.splice(at, 1);
        else CONFIG._flagged.push({
          rowId: ev.rowId, date: ev.date, actor: ev.actor,
          side: ev.side, category: ev.category,
          desc: (ev.descHeMedium || '').slice(0, 90)
        });
        save();
        if (CONFIG._sync) CONFIG._sync();
        paint();
      ...
      window.addEventListener('click', function (e) {
        if (!onFold13()) return;
        if (p9.anim) return;
        var sticky = document.querySelector('.page9-sticky');
        if (sticky && sticky.classList.contains('dragging')) return;
        if (e.target.closest && e.target.closest('.page9-pill, .hp, .hp-chip')) return;
        if (!p9.hoveredEvent) return;
        toggle(p9.hoveredEvent);
      }, true);
    },

    /* The flagged list, in the panel: what was clicked, what it is currently
       classified as, and a way to take one back off the list. */
    custom: function (box, api, doc) {
      box.innerHTML = '';
      var head = doc.createElement('div');
      head.style.cssText = 'font-size:11px;opacity:.7;margin:2px 0 6px;line-height:1.5';
      head.textContent = 'On @fold13: arrow keys move between dots, Enter or Space flags the '
        + 'selected one, and clicking a dot does the same. Repeat to unflag.';
      box.appendChild(head);

      var list = doc.createElement('div');
      list.style.cssText = 'max-height:260px;overflow:auto;display:flex;flex-direction:column;gap:5px';
      box.appendChild(list);

      var foot = doc.createElement('div');
      foot.style.cssText = 'display:flex;gap:8px;align-items:center;margin-top:8px;font-size:11px';
      var count = doc.createElement('span');
      var clear = doc.createElement('button');
      clear.textContent = 'clear all';
      clear.style.cssText = 'font:inherit;padding:2px 8px;cursor:pointer';
      clear.onclick = function () {
        if (!CONFIG._flagged.length) return;
        CONFIG._flagged.length = 0; CONFIG._save(); render(); CONFIG._paint();
      };
      foot.appendChild(count); foot.appendChild(clear);
      box.appendChild(foot);

      function render() {
        var f = CONFIG._flagged || [];
        count.textContent = f.length + (f.length === 1 ? ' dot flagged' : ' dots flagged');
        list.innerHTML = '';
        f.forEach(function (d, i) {
          var row = doc.createElement('div');
          row.style.cssText = 'display:flex;gap:6px;align-items:flex-start;font-size:11px;'
            + 'line-height:1.45;padding:4px 6px;border-radius:4px;background:rgba(255,255,255,.05)';
          var x = doc.createElement('button');
          x.textContent = '×';
          x.title = 'remove from the list';
          x.style.cssText = 'font:inherit;cursor:pointer;flex:none;padding:0 5px';
          x.onclick = function () {
            CONFIG._flagged.splice(i, 1); CONFIG._save(); render(); CONFIG._paint();
          };
          var txt = doc.createElement('div');
          txt.style.cssText = 'flex:1;min-width:0';
          txt.innerHTML = '<b>' + d.rowId + '</b> · ' + d.date
            + '<br><span style="opacity:.75">' + d.actor + ' → <b>' + d.category + '</b></span>'
            + '<br><span dir="rtl" style="opacity:.55;display:block">' + d.desc + '</span>';
          row.appendChild(x); row.appendChild(txt);
    ...
    /* What the panel tab shows, live. Same content as Copy, so the tab is a
       running view of exactly what will land in chat. */
    readout: function () {
      var f = (CONFIG && CONFIG._flagged) || [];
      if (!f.length) return 'nothing flagged yet — hover a dot on @fold13 and click it';
      return f.map(function (d, i) {
        return (i + 1) + '. ' + d.rowId + '  ' + d.date + '  [' + d.side + '] '
             + d.actor + '\n    now: ' + d.category;
      }).join('\n');
    },

    goTo: '#page-12', goLabel: '@fold13', onGo: null,
    width: 380, collapseSliders: true, knobCols: 1,

    /* Copy emits the list in a form that is useful on the other end: the row
       ids to look up, and each one's CURRENT classification, which is the
       thing being called wrong. */
    summary: function () {
      var f = CONFIG._flagged || [];
      if (!f.length) return 'no dots flagged';
      var out = ['misclassified — ' + f.length + ' dot(s) flagged on @fold13', ''];
      f.forEach(function (d) {
        out.push(d.rowId + '  ' + d.date + '  [' + d.side + '] ' + d.actor
               + '  currently: ' + d.category);
        if (d.desc) out.push('    ' + d.desc);
      });
      out.push('', 'rowIds: ' + f.map(function (d) { return d.rowId; }).join(', '));
      return out.join('\n');
    }
  };
  // ------------------------------------------------------------ END CONFIG --
  ...
  var API = {
    /* Push the readout to the remote tab. A harness that COLLECTS (rather than
       tunes) has its result in that text, so it must reach the tab the user is
       actually looking at — custom UI never crosses the bus, this does. */
