...
(function () {
  ...
  var CONFIG = {
    title: 'mlegend-acled',
    label: 'the ACLED note in the legend',
    fold: '@fold6',
    viewport: 'mobile',
    remoteOnly: true,
    tabs: [], tab: null, onTab: null,

    /* «איסוף הנתונים» — the collapsible section at the foot of the legend. The
       size button above it is now an outline card in the group cards' shape;
       these give the note the same treatment. Open the legend and TAP the
       heading to see the body, which is collapsed by default. */
    modes: [
      { key: '1', id: 'head',   label: '1 — the heading alone is an outline card; the text flows beneath it' },
      { key: '2', id: 'both',   label: '2 — heading and text in ONE outline card together' },
      { key: '3', id: 'filled', label: '3 — the heading as a FILLED card, like the group cards' },
    ],
    mode: null,
    toggles: [
      { key: 'r', id: 'keepRule', label: 'keep the rule above it', on: true },
    ],

    sliders: [
      { key: 'gap',    label: 'space above it', min: 0, max: 48, step: 1, value: 8,
        source: 'style.css — .fold6-mlegend-data-head margin-top' },
      { key: 'radius', label: 'how round it is', min: 0, max: 28, step: 1, value: 10,
        source: 'style.css — .fold6-mlegend-data-head border-radius' },
      { key: 'padY',   label: 'space inside it, above and below', min: 0, max: 28, step: 1, value: 12,
        source: 'style.css — .fold6-mlegend-data-head padding (vertical)' },
      { key: 'padX',   label: 'space inside it, left and right', min: 0, max: 36, step: 1, value: 13,
        source: 'style.css — .fold6-mlegend-data-head padding (horizontal)' },
      { key: 'border', label: 'how thick the outline is', min: 0.5, max: 4, step: 0.5, value: 1,
        modes: ['head', 'both'],
        source: 'style.css — .fold6-mlegend-data-head border-width' },
    ],
    colors: [
      { key: 'line', label: 'the outline\'s colour', value: '#c9c9c9',
        modes: ['head', 'both'],
        source: 'style.css — .fold6-mlegend-data-head border-color' },
      { key: 'fill', label: 'its fill', value: '#e1e1e1',
        modes: ['filled'],
        source: 'style.css — .fold6-mlegend-data-head background' },
    ],

    apply: function (v, mode, tab, on) {
      var css = '';
      /* The rule above the section is the shared .fold6-mlegend-divider. Once
         the note is a card of its own it is usually one line too many, so it is
         a toggle rather than part of a candidate. */
      if (!on.keepRule) css += '.fold6-mlegend-divider{display:none}';

      if (mode) {
        var box = 'box-sizing:border-box;margin-top:' + v.gap + 'px!important;'
          + 'padding:' + v.padY + 'px ' + v.padX + 'px!important;';
        /* The chevron is positioned off the heading's left edge, so the new
           horizontal padding has to move it in by the same amount or it sits on
           the outline. */
        css += '.fold6-mlegend-data-head::after{left:' + (v.padX - 7) + 'px}';

        if (mode === 'filled') {
          css += '.fold6-mlegend-data-head{' + box + 'border:0!important;'
            + 'border-radius:' + v.radius + 'px;background:' + v.fill + '!important;'
            + 'box-shadow:0 1px 0 rgba(0,0,0,.07)}';
        } else if (mode === 'head') {
          css += '.fold6-mlegend-data-head{' + box
            + 'border:' + v.border + 'px solid ' + v.line + '!important;'
            + 'border-radius:' + v.radius + 'px;background:none!important}';
        } else {
          /* ONE card round both. The body is a sibling, not a child, so the two
             are drawn as halves of the same box: the heading loses its bottom
             corners and the body picks them up, and the body's own top border
             is dropped so the seam does not double up. The body is
             `height: 0; overflow: hidden` when collapsed, so its border only
             appears once it is open — which is what makes this read as one card
             that grows rather than two stacked. */
          css += '.fold6-mlegend-data-head{' + box
            + 'border:' + v.border + 'px solid ' + v.line + '!important;border-bottom:0!important;'
            + 'border-radius:' + v.radius + 'px ' + v.radius + 'px 0 0;background:none!important}'
          + '.fold6-mlegend-data-body{box-sizing:border-box;'
            + 'border:' + v.border + 'px solid ' + v.line + ';border-top:0;'
            + 'border-radius:0 0 ' + v.radius + 'px ' + v.radius + 'px;'
            + 'padding:0 ' + v.padX + 'px}';
        }
      }
      CONFIG.style.textContent = css;
      if (typeof fold6MLegendPaintCard === 'function') fold6MLegendPaintCard(fold6MLegendOpenRaw);
    },

    init: null, custom: null,
    goTo: '#page-5', goLabel: '@fold6', onGo: null,
    width: 0, collapseSliders: false, knobCols: 1,
    summary: function (v, mode, tab, on) {
      return 'ACLED note: ' + (mode || 'current') + ' · rule ' + (on.keepRule ? 'kept' : 'off');
    }
  };
  // ------------------------------------------------------------ END CONFIG --
