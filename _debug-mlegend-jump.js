...
(function () {
  ...
  var CONFIG = {
    title: 'mlegend-jump',
    label: 'legend: the pop-up on the ACLED crossing',
    fold: '@fold6',
    viewport: 'mobile',
    remoteOnly: true,
    ...
    /* The closed מקרא pill jumps up a little and flashes on @fold6's ACLED
       crossing — and again 1s after @fold11's dots land. Go replays it. */
    toggles: [
      { key: 'f', id: 'flash', label: 'flash a darker fill while it jumps', on: true },
    ],
    sliders: [
      { key: 'jump',   label: 'how high it jumps', min: 0, max: 160, step: 1, value: 52,
        source: 'js/groups.js — FOLD6_MLEGEND_JUMP_PX' },
      { key: 'ms',     label: 'how long the jump takes, up and back (ms)', min: 100, max: 2000, step: 10, value: 700,
        source: 'js/groups.js — FOLD6_MLEGEND_JUMP_MS' },
      { key: 'handle', label: 'the grab line: gap from the top of the closed pill', min: 0, max: 24, step: 1, value: 5,
        source: 'js/groups.js — FOLD6_MLEGEND_HANDLE_TOP_PX' },
      { key: 'flashMs', label: 'how long the flash lasts (ms)', min: 100, max: 2000, step: 10, value: 640,
        source: 'js/groups.js — FOLD6_MLEGEND_FLASH_MS' },
      { key: 'delay',  label: '@fold11: wait after the dots land before it jumps (ms)', min: 0, max: 4000, step: 50, value: 1000,
        source: 'js/groups.js — FOLD11_LEGEND_JUMP_DELAY_MS' },
    ],
    colors: [
      { key: 'flashColor', label: 'the flash colour', value: '#e6e4ec',
        source: 'js/groups.js — FOLD6_MLEGEND_FLASH_COLOR' },
    ],

    apply: function (v, mode, tab, on) {
      FOLD6_MLEGEND_JUMP_PX = v.jump;
      FOLD6_MLEGEND_JUMP_MS = v.ms;
      FOLD6_MLEGEND_HANDLE_TOP_PX = v.handle;
      FOLD6_MLEGEND_FLASH_MS = v.flashMs;
      FOLD6_MLEGEND_FLASH_COLOR = v.flashColor;
      FOLD6_MLEGEND_FLASH_ON = !!on.flash;
      FOLD11_LEGEND_JUMP_DELAY_MS = v.delay;
    ...
    /* Replay the jump on arrival (the pill must be closed for it to play). */
    onGo: function () {
      if (typeof fold6SetMobileLegendOpen === 'function' && fold6MLegendOpenWant) fold6SetMobileLegendOpen(false);
      setTimeout(function () { if (typeof fold6MLegendJump === 'function') fold6MLegendJump(true); }, 500);
    },
    width: 0, collapseSliders: false, knobCols: 1,
    summary: function (v, mode, tab, on) {
      return 'var FOLD6_MLEGEND_JUMP_PX = ' + v.jump + ';\n'
        + 'var FOLD6_MLEGEND_JUMP_MS = ' + v.ms + ';\n'
        + 'var FOLD6_MLEGEND_HANDLE_TOP_PX = ' + v.handle + ';\n'
        + 'var FOLD6_MLEGEND_FLASH_ON = ' + (on.flash ? 'true' : 'false') + ';\n'
        + 'var FOLD6_MLEGEND_FLASH_COLOR = "' + v.flashColor + '";\n'
        + 'var FOLD6_MLEGEND_FLASH_MS = ' + v.flashMs + ';\n'
        + 'var FOLD11_LEGEND_JUMP_DELAY_MS = ' + v.delay + ';';
    }
  };
  // ------------------------------------------------------------ END CONFIG --
