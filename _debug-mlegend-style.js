...
(function () {
  ...
  var CONFIG = {
    title: 'mlegend-style',
    label: 'legend: ACLED note, resize button, chevron',
    fold: '@fold6',
    viewport: 'mobile',
    remoteOnly: true,
    tabs: [], tab: null, onTab: null,

    /* Two independent choices (one per group) plus one knob.
       - the ACLED note card («איסוף הנתונים») and the resize button
         («הצגת גודל האירועים», @fold11 on) are OUTLINE cards today; `1`/`2`
         restyle each like the six group rows (filled #e1e1e1, no border,
         the 1px shadow). `0` in a group = the outline it ships with.
       - the chevron's gap from the note card's left edge is a knob either way. */
    modes: [
      { key: '1', id: 'noteRow', label: 'filled like the group rows', group: 'ACLED note' },
      { key: '2', id: 'btnRow',  label: 'filled like the group rows', group: 'resize button' },
    ],
    offLabels: { 'ACLED note': 'outline (current)', 'resize button': 'outline (current)' },
    mode: null,
    toggles: [],

    sliders: [
      { key: 'chev', label: 'chevron: gap from the card\'s left edge', min: 0, max: 40, step: 1, value: 12,
        source: 'style.css — .fold6-mlegend-data-head::after left (≤600px)' },
      { key: 'notePadY', label: 'ACLED note: space above and below the text', min: 0, max: 28, step: 1, value: 12,
        modes: ['noteRow'], source: 'style.css — .fold6-mlegend-data-card padding (vertical, ≤600px)' },
      { key: 'btnPadY', label: 'resize button: space above and below its text', min: 0, max: 28, step: 1, value: 12,
        modes: ['btnRow'], source: 'style.css — .fold6-mlegend-scope padding (vertical, ≤600px)' },
    ],
    colors: [
      { key: 'fill', label: 'the filled cards\' colour', value: '#e1e1e1', modes: ['noteRow', 'btnRow'],
        source: 'style.css — .fold6-mlegend-row background (the rows\' own fill)' },
    ],

    apply: function (v, mode, tab, on) {
      var m = mode || {};
      var css = '.fold6-mlegend-data-head::after{left:' + v.chev + 'px}';
      var row = function (sel, padY) {
        return sel + '{background:' + v.fill + '!important;border:none!important;'
          + 'box-shadow:0 1px 0 rgba(0,0,0,.07);padding:' + padY + 'px 13px!important}';
      };
      if (m['ACLED note'] === 'noteRow')   css += row('.fold6-mlegend-data-card', v.notePadY);
      if (m['resize button'] === 'btnRow') css += row('.fold6-mlegend-scope', v.btnPadY);
    ...
    goTo: '#page-5', goLabel: '@fold6',
    /* Open the sheet so the cards are on screen; the resize button only exists
       from @fold11's crossing on — scroll there to judge it. */
    onGo: function () {
      if (typeof fold6SetMobileLegendOpen === "function") fold6SetMobileLegendOpen(true);
      if (typeof fold6MDataToggle === "function" && typeof fold6MobileDataBodyEl !== "undefined" && fold6MobileDataBodyEl.hidden === false) fold6MDataToggle(true);
    },
    width: 0, collapseSliders: false, knobCols: 1,
    summary: function (v, mode) {
      var m = mode || {};
      return 'ACLED note: ' + (m['ACLED note'] === 'noteRow' ? 'filled like the rows' : 'outline')
        + ' · resize button: ' + (m['resize button'] === 'btnRow' ? 'filled like the rows' : 'outline');
    }
  };
  // ------------------------------------------------------------ END CONFIG --
