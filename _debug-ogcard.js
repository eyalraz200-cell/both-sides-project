...
(function () {
  ...
  var CONFIG = {
    title: 'ogcard',
    label: 'og card — size & palette',
    fold: null,
    viewport: 'desktop',
    /* Docked, and it STAYS docked while the panel tab is open. This page is
       nothing but the artboard plus grey surround, so the panel sits on the
       surround rather than over the card, and there is no scrolling to do. */
    remoteOnly: false,
    keepDocked: true,
    tabs: [], tab: null, onTab: null,

    /* Two independent choices, so they are two mode GROUPS rather than one
       list: the four Figma frames differ ONLY in scale (dot, pitch, column
       gap, card width), and the palette is orthogonal to all of it. Picking a
       size preset RESETS the four sliders to that frame's measured values —
       then turn them freely; you are no longer on a preset at that point. */
    modes: [
      { key: 'a', id: 'A', label: 'A · 360-4558', group: 'size' },
      { key: 'b', id: 'B', label: 'B · 360-4146', group: 'size' },
      { key: 'c', id: 'C', label: 'C · 360-3093', group: 'size' },
      { key: 'd', id: 'D', label: 'D · 360-3939', group: 'size' },
      { key: '5', id: 'figma',   label: 'Figma stand-ins',   group: 'colour' },
      { key: '6', id: 'project', label: 'project groups',    group: 'colour' },
      { key: '7', id: 'split',   label: 'split — camp | camp', group: 'colour' },
      { key: '8', id: 'mono',    label: 'ink only',          group: 'colour' },
      { key: 'l', id: 'thin',    label: 'Thin (300)',    group: 'weight' },
      { key: 'r', id: 'regular', label: 'Regular (400)', group: 'weight' }
    ],
    mode: { size: 'B', colour: 'project', weight: 'regular' },
    toggles: [],

    sliders: [
      { key: 'dot',    label: 'size of each dot', min: 4, max: 22, step: 0.5, value: 9,
        source: '_og-card.html — #card --dot' },
      { key: 'pitch',  label: 'gap down a column', min: 8, max: 46, step: 0.5, value: 24,
        source: '_og-card.html — #card --pitch (centre to centre)' },
      { key: 'colgap', label: 'gap between the two columns', min: 14, max: 120, step: 0.5, value: 34,
        source: '_og-card.html — #card --colgap (centre to centre)' },
      { key: 'cardw',  label: 'title card width', min: 460, max: 1000, step: 1, value: 702,
        source: '_og-card.html — #card --cardw' },
      { key: 'cardr',  label: 'title card corner rounding', min: 0, max: 60, step: 1, value: 24,
        source: '_og-card.html — #card --cardr' },
      { key: 'fs',     label: 'title size', min: 30, max: 84, step: 0.5, value: 56,
        source: '_og-card.html — #card --fs' },
      { key: 'dashscale', label: 'dash thickness & spacing', min: 1, max: 8, step: 0.25, value: 2,
    ...
    /* Picking a size preset writes its measured numbers back into the sliders,
       so the panel never shows values that disagree with the card. */
    _lastSize: null,
    /* Every card this page controls. On the card page itself that is `window`;
       on the mockup it is the four same-origin iframes, all driven from the one
       panel so the previews and the full-size card never disagree. */
    targets: function () {
      var out = [];
      if (window.OG_RENDER) out.push(window);
      Array.prototype.forEach.call(document.querySelectorAll('iframe'), function (f) {
        try { if (f.contentWindow && f.contentWindow.OG_RENDER) out.push(f.contentWindow); }
        catch (e) { /* cross-origin: not ours */ }
      });
      return out;
    },
    apply: function (v, mode) {
      var wins = CONFIG.targets();
      if (!wins.length) return;
      var W = wins[0];
      var size = (mode && mode.size) || null;
      if (size && size !== CONFIG._lastSize && W.OG_PRESETS[size]) {
        var pre = W.OG_PRESETS[size];
        ['dot', 'pitch', 'colgap', 'cardw', 'cardr', 'fs'].forEach(function (k) {
          v[k] = pre[k];
          CONFIG.sliders.forEach(function (s) { if (s.key === k) s.value = pre[k]; });
        });
        CONFIG._lastSize = size;
        if (CONFIG._api) setTimeout(function () { CONFIG._api.refresh(); }, 0);
      }
      var args = {
        palette: (mode && mode.colour) || 'figma',
        dot: v.dot, pitch: v.pitch, colgap: v.colgap,
        cardw: v.cardw, cardr: v.cardr, fs: v.fs,
        fw: (mode && mode.weight) === 'regular' ? 400 : 300,
        extras: v.extras,
        dashscale: v.dashscale,
        seed: 20260917 + v.seed * 104729
      };
      wins.forEach(function (w) { try { w.OG_RENDER(args); } catch (e) {} });
    },

    init: function (api) {
      CONFIG._api = api;
      /* An iframe that finishes loading after the first apply would sit on its
         own defaults — re-apply as each one arrives. */
      Array.prototype.forEach.call(document.querySelectorAll('iframe'), function (f) {
        f.addEventListener('load', function () { api.refresh(); });
      });
    },
    custom: null,
    goTo: '#card', goLabel: 'card', onGo: null,
    width: 360, collapseSliders: false, knobCols: 1,

    summary: function (v, mode) {
      return 'size preset: ' + ((mode && mode.size) || 'custom')
        + '\npalette: ' + ((mode && mode.colour) || 'figma')
        + '\ndot ' + v.dot + '  pitch ' + v.pitch + '  colgap ' + v.colgap
        + '\ncard ' + v.cardw + 'x auto  r' + v.cardr + '  title ' + v.fs
        + ' / ' + ((mode && mode.weight) === 'regular' ? 'Regular 400' : 'Thin 300')
        + '\ndash scale: ' + v.dashscale + '  (1 = site parity)'
        + '\nextra colours: ' + v.extras
        + '\nseed ' + v.seed;
    }
  };
  // ------------------------------------------------------------ END CONFIG --
