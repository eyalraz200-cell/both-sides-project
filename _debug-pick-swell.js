...
(function () {
  ...
  var CONFIG = {
    title: 'pick-swell',
    label: 'the picked dot: swell time by size',
    fold: '@fold9',
    viewport: 'mobile',
    remoteOnly: true,
    ...
    /* The swell (and the push around it, and the collapse on the way out) runs
       on a clock that grows with the dot's size: the smallest swelling dots take
       the first knob; every size step above adds the second. The five sizes are
       2.4x, 3.9x, 8.1x, 12.4x and 19.6x the dot. Copy lists what each size gets. */
    sliders: [
      { key: 'base',  label: 'smallest swelling dots: how long (ms)', min: 40, max: 1500, step: 10, value: 200,
        source: 'page7.js — P7_BULGE_MS_MOBILE' },
      { key: 'per',   label: 'each size step above that adds (ms)', min: 0, max: 600, step: 10, value: 0,
        source: 'page7.js — P7_BULGE_MS_PER_TIER_MOBILE' },
    ],
    colors: [],

    apply: function (v) {
      P7_BULGE_MS_MOBILE = v.base;
      P7_BULGE_MS_PER_TIER_MOBILE = v.per;
    },

    init: null, custom: null, timeline: null,
    goTo: '#page-8', goLabel: '@fold9', onGo: null,
    width: 0, collapseSliders: false, knobCols: 1,
    summary: function (v) {
      var sizes = ['2.4x', '3.9x', '8.1x', '12.4x', '19.6x'];
      var lines = sizes.map(function (s, i) { return '  ' + s + ' dot: ' + (v.base + v.per * i) + 'ms'; });
      return lines.join('\n') + '\nvar   P7_BULGE_MS_MOBILE  = ' + v.base + ';\nvar   P7_BULGE_MS_PER_TIER_MOBILE = ' + v.per + ';';
    }
  };
  // ------------------------------------------------------------ END CONFIG --
