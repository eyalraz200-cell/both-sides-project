...
(function () {
  ...
  var CONFIG = {
    title: 'tip-flip',
    label: 'where the tooltip flips sides',
    fold: '@fold9',
    viewport: 'desktop',
    remoteOnly: true,
    tabs: [
      { id: 'f9',  label: '@fold9 timeline' },
      { id: 'f10', label: '@fold10 size grid' },
      { id: 'f13', label: '@fold13 columns' }
    ],
    goTo: function () { var t = (CONFIG.api && CONFIG.api.tab()) || 'f9'; return document.querySelector({ f9: '#page-8', f10: '#page-9', f13: '#page-12' }[t]); },
    goLabel: 'the tab\'s fold',
    sliders: [
      /* ONE knob per fold: the two lines are mirrored — the same distance in
         from each edge (explicit instruction). */
      { key: 'inset', label: 'flip lines, distance in from each edge (px)', min: 0, max: 800, step: 1,
        value: 327, ship: 327, source: 'page7.js P7_TIP_FLIP_L_F9 + _R_INSET_F9 / _F10 · page9.js P9_TIP_FLIP_L + _R_INSET' }
    ],
    apply: function (v, mode, tab) {
      if (typeof P7_TIP_FLIP_L_F9 === 'undefined') return;
      P7_TIP_FLIP_L_F9  = P7_TIP_FLIP_R_INSET_F9  = v.f9.inset;
      P7_TIP_FLIP_L_F10 = P7_TIP_FLIP_R_INSET_F10 = v.f10.inset;
      if (typeof P9_TIP_FLIP_L !== 'undefined') P9_TIP_FLIP_L = P9_TIP_FLIP_R_INSET = v.f13.inset;
      CONFIG.paintMarks && CONFIG.paintMarks();
    },
    /* Trigger markers: two short ticks in the TOP margin at the live flip lines
       of the fold on screen (currentPage picks which pair), each labelled, coloured
       by the pointer: grey = armed, orange = the pointer is past that line (the
       tooltip is being forced that way). Short ticks, never a full-height rule. */
    init: function (api) {
      CONFIG.api = api;
      var box = document.createElement('div');
      box.style.cssText = 'position:fixed;left:0;top:0;width:100%;height:0;pointer-events:none;z-index:2147483000;font:11px/1 -apple-system,Helvetica,sans-serif;';
      function tick(lbl) { var t = document.createElement('div'); t.style.cssText = 'position:absolute;top:0;width:2px;height:18px;background:#999;'; var l = document.createElement('div'); l.style.cssText = 'position:absolute;top:20px;left:-30px;width:62px;text-align:center;color:#666;white-space:nowrap;'; l.textContent = lbl; t.appendChild(l); box.appendChild(t); return t; }
      var tl = tick('L flip'), tr = tick('R flip');
      document.body.appendChild(box);
      var px = -1;
      document.addEventListener('mousemove', function (e) { px = e.clientX; paint(); }, { passive: true });
      function pair() {
        var cp = typeof currentPage === 'number' ? currentPage : 8;
        if (cp === 12) return [P9_TIP_FLIP_L, P9_TIP_FLIP_R_INSET];
        if (cp === 9)  return [P7_TIP_FLIP_L_F10, P7_TIP_FLIP_R_INSET_F10];
        return [P7_TIP_FLIP_L_F9, P7_TIP_FLIP_R_INSET_F9];
      }
      function paint() {
        var p = pair(); var xl = p[0], xr = window.innerWidth - p[1];
        tl.style.left = xl + 'px'; tr.style.left = xr + 'px';
        tl.style.background = (px >= 0 && px < xl) ? '#e8892b' : '#999';
        tr.style.background = (px >= 0 && px > xr) ? '#e8892b' : '#999';
      }
      CONFIG.paintMarks = paint;
