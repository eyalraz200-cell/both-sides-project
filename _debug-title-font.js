/* SCAFFOLDING — compare/ title-block fonts + sizes. Never ships.
   Delete this file and its entry in index.html's _debug block when decided. */
(function () {
  var FONTS = [
    ['2', 'david',   'David Libre',          "'David Libre', serif"],
    ['9', 'miriam',  'Miriam Libre',         "'Miriam Libre', sans-serif"],
    ['q', 'plex',    'IBM Plex Sans Hebrew', "'IBM Plex Sans Hebrew', sans-serif"],
    ['a', 'assistant', 'Assistant',          "'Assistant', sans-serif"]
  ];
  var FAMILY = {}; FONTS.forEach(function (f) { FAMILY[f[1]] = f[3]; });
  var link = document.createElement('link');
  link.rel = 'stylesheet';
  link.href = 'https://fonts.googleapis.com/css2?family=Frank+Ruhl+Libre:wght@300..900' +
    '&family=David+Libre:wght@400;500;700&family=Noto+Serif+Hebrew:wght@100..900' +
    '&family=Bellefair&family=Suez+One&family=Tinos:wght@400;700' +
    '&family=Miriam+Libre:wght@400..700&family=IBM+Plex+Sans+Hebrew:wght@100;200;300;400;500;600;700&display=swap';
  document.head.appendChild(link);
  var face = document.createElement('style');
  face.textContent = "@font-face{font-family:'Hadasim CLM Local';font-weight:400;src:local('Hadasim CLM'),local('HadasimCLM-Regular');}" +
    "@font-face{font-family:'Hadasim CLM Local';font-weight:700;src:local('Hadasim CLM Bold'),local('HadasimCLM-Bold');}";
  document.head.appendChild(face);
  // ---------------------------------------------------------------- CONFIG --
  var CONFIG = {
    title: 'title-font',
    label: 'title block font & size',
    fold: '@fold5',
    viewport: 'both',
    remoteOnly: true,
    tabs: [], tab: null, onTab: null,
    sliders: [
      { key: 'sizeD', label: 'title size on desktop', min: 14, max: 32, step: 1, value: 20,
      ...
      { key: 'lh', label: 'space between lines', min: 1.1, max: 2, step: 0.05, value: 1.5,
        source: 'style.css:1958 — .section-title line-height' }
    ],
    colors: [],
    modes: FONTS.map(function (f) { return { key: f[0], id: f[1], label: f[2] }; }),
    mode: null,
    toggles: [],
    apply: function (v, mode) {
      var fam = mode ? 'font-family:' + FAMILY[mode] + ';font-weight:' + v.weight + ';' : '';
      CONFIG.style.textContent =
        '.section-title{' + fam + 'font-size:' + v.sizeD + 'px;line-height:' + v.lh + ';}' +
        '#page-16 .section-title{' + fam + '}' +
        '@media (max-width:600px){.section-title{font-size:' + v.sizeM + 'px;}}';
      if (typeof updateTextCardFrameDashes === 'function') requestAnimationFrame(function () { updateTextCardFrameDashes(); });
    },
    init: null, custom: null, timeline: null,
    goTo: '#page-4', goLabel: '@fold5', onGo: null,
    width: 0, collapseSliders: false, knobCols: 1,
    summary: function (v, mode) {
      return 'font: ' + (mode ? FAMILY[mode] + ' @ ' + v.weight : 'current (HadassahFriedlaender 100)') +
        '\ndesktop ' + v.sizeD + 'px · phone ' + v.sizeM + 'px · line-height ' + v.lh;
    }
  };
  // ------------------------------------------------------------ END CONFIG --
