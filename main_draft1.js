...
function drawScreen3() {
  ...
  ctx.fillStyle = "#555";
  ctx.fillText("כלפי מי הופנה האירוע", W / 2, PAD_TOP / 2 + 12);

  const TARGET_HE = {
    "Government":     "ממשלת ישראל",
    "Property":       "רכוש",
...
function drawScreen4() {
  ...
  ctx.font = "11px monospace";
  ctx.fillStyle = "#555";
  ctx.fillText("רכוש שנפגע באירוע", W / 2, PAD_TOP / 2 + 12);
...
function drawScreen5() {
  ...
  ctx.font = "11px monospace";
  ctx.fillStyle = "#555";
  ctx.fillText("אמצעים ששימשו באירוע", W / 2, PAD_TOP / 2 + 12);
...
function drawScreen1b() {
  ...
  const idfTotal = Object.values(IDF_COUNTS).reduce((a, b) => a + b, 0);
  ctx.textBaseline = "bottom";
  ctx.fillText("אירועים בהם הצטרפו כוחות הביטחון", rightOuterX + 12, rightTopY - spacing / 2);
  ctx.textBaseline = "top";
  ctx.fillText(idfTotal.toLocaleString(), rightOuterX + 12, rightTopY + spacing / 2);
}
...
function drawScreen0b() {
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "bottom";
  ctx.fillText("כל עיגול מייצג אירוע חריג מהקצה הפוליטי", mid, H / 2 + 160);

  const dotY = H / 2 + 200 + r + 12;

  ctx.beginPath();
  ctx.arc(mid - spacing / 2 - CENTER_PAD, dotY, r, 0, Math.PI * 2);
...
function drawScreen0c() {
  ...
  ctx.textAlign = "right";
  ctx.textBaseline = "middle";
  ctx.fillText("שמאל", mid - spacing / 2 - CENTER_PAD - r - 6, dotY);
  ...
  ctx.fillStyle = "#111";
  ctx.textAlign = "left";
  ctx.fillText("ימין", mid + spacing / 2 + CENTER_PAD + r + 6, dotY);
}
...
function drawScreen10() {
  ...
  // Bottom text after last dot
  const textBaseY = PAD_TOP + spacing / 2 + (screen10Dots.length - 1) * spacing + spacing * 150;
  const lastY = textBaseY - screen10ScrollY;
  if (lastY > PAD_TOP && lastY < H) {
    ctx.font = "13px monospace";
    ctx.fillStyle = "#111";
    ctx.textAlign = "center";
    ctx.textBaseline = "top";
    ctx.fillText("קיצוניים משני הצדדים", x, lastY);
  }
...
function render() {
  if (currentScreen === 0) drawScreenDraft();
  else if (currentScreen === 1) drawScreen0();
  else if (currentScreen === 2) drawScreen0b();
  else if (currentScreen === 3) drawScreen0c();
  else if (currentScreen === 4) drawScreen1();
  else if (currentScreen === 5) drawScreen1b();
  else if (currentScreen === 6) drawScreen2();
  else if (currentScreen === 7) drawScreen3();
  else if (currentScreen === 8) drawScreen5();
  else if (currentScreen === 9) drawScreen4();
  else if (currentScreen === 10) drawScreenInjury3();
  else drawScreen10();
}
...
window.addEventListener("wheel", (e) => {
  if (currentScreen === 11) {
    const textBaseY = PAD_TOP + spacing / 2 + (screen10Dots.length - 1) * spacing + spacing * 150;
    const maxScroll = Math.max(0, textBaseY - canvas.height / 2);
    if (e.deltaY > 0) {
    ...
    render();
  } else if (e.deltaY > 0 && currentScreen < 11) {
    currentScreen++;
    render();
