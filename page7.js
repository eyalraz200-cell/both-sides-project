...
const p7 = {
  ...
  // The event currently under the pointer in #page-9 (set by p7HoverInit's
...
function p7TipFlipPair(page) {
  const f = page === 9 ? P7_TIP_FLIP_FRAC_F9 : page === 13 ? P9_TIP_FLIP_FRAC : P7_TIP_FLIP_FRAC_F10;
...
// The date the timeline RESTS at before it has engaged — one day before the
// dataset's own first day, so that NOTHING is reached yet. p7.minDate itself
// would not do: p7RowEndOfDate(minDate) is rowStart[0] + rowsOf[0], i.e. day
...
function p7RowEndOfDate(dateStr) {
  const v = p7.vert; if (!v) return 0;
  if (p7DayMs(dateStr) > v.maxMs) return v.totalRows;
  // Before the dataset begins — the pre-engagement rest state (p7PreStartDate).
  // v.dayOf clamps negatives to day 0, so without this the rest state would
  // report day one's rows as reached.
  if (p7DayMs(dateStr) < v.minMs) return 0;
  const d = v.dayOf(dateStr);
...
// #page-10's mobile padding-top (style.css, max-width 600px) — and neither can be
...
// viewport-height *after* the card is already gone, leaving a stretch of
// scrolling where nothing visibly happens before the real per-event reveal
// kicks in. Tying engagement to the card's own crossing instead means the timeline
// starts exactly when the title that introduces it leaves the screen, no
// matter how #page-8's section ends up being sized.
...
// of view), which left a stretch of dead scroll between the title vanishing and
// the fill starting — half a card-height of it, plus however much of the
// section remained. 0 would be the instant its top edge touches the top.
// page7UpdateFromScroll (js/page7-scrub.js) starts its t=0 at the same line, so
// the dataset still begins at its first day and the scrub's pace is unchanged.
// Per breakpoint, both 0.5 today.
const P7_ENGAGE_CARD_OUT_FRAC_DESKTOP = 0.5;
const P7_ENGAGE_CARD_OUT_FRAC_MOBILE  = 0.5;
function p7EngageOffsetPx() {
  if (typeof fold9AxisCardEl === "undefined" || !fold9AxisCardEl) return 0;
  const f = isMobile() ? P7_ENGAGE_CARD_OUT_FRAC_MOBILE : P7_ENGAGE_CARD_OUT_FRAC_DESKTOP;
  return f * fold9AxisCardEl.getBoundingClientRect().height;
}
...
function p7AxisFillFracTarget() {
  ...
  // Clamped at 0: the pre-engagement rest date (p7PreStartDate) is a day
  // BEFORE minDate, which would otherwise read as a slightly negative fill.
  return maxMs === minMs ? 0 : Math.max(0, (curMs - minMs) / (maxMs - minMs));
}
...
function p7GridLiveRect(ev, isLeft) {
  if (!p7Grid.on) return null;
  // The pack is normally built by drawPage7 alone. A viewport change
  // (p7UpdateLayout) or an instant re-sync can drop it while page8's glide is
  // in the air and drawPage7 is not the one painting — rebuild rather than
  // fall back to the timeline cell, which is a snap. Last resort, not the
  // normal path: it only differs from drawPage7's pack while p7.lastPositions
  // is partial.
  if (!p7Grid.layout && typeof canvas !== "undefined") p7SizeGridLayout(canvas.clientWidth, canvas.clientHeight);
  if (!p7Grid.layout) return null;
...
function p7SizeGridOnPage(page) {
  ...
  if (page < 10 && typeof p7ScopeUserUniform !== "undefined") p7ScopeUserUniform = null;
  // Snapshot for the p9 sync below: only a flip that actually MOVES the flag
  // (or leaves the band upward) may throw the morph and the held plan away.
  const uniformWas = p7GridUniform;
...
function p7DrawSideSquares(ctx, events, positions, x0, topY, cols, CELL, SQ, monthEnd, settledCount, posMap) {
  ...
  for (let i = 0; i < monthEnd; i++) {
    ...
    } else if (typeof fold6LegendHoverDimT === "function") {
      // A hovered legend ROW (js/groups.js): its group stays at full alpha,
      // everything else dims to the same floor. Lowest priority — a hovered
      // dot (the branch above) wins.
      const lt = fold6LegendHoverDimT();
      if (lt > 0 && events[i].actor !== fold6LegendHoverActor)
        drawAlpha = alpha * (1 - (1 - hoverDim(events[i].actor)) * lt);
    }
...
function p7DrawTimelineSquares(ctx, W, H) {
  ...
  // pinned before the first month (p7.currentDate = p7PreStartDate(), page7UpdateFromScroll),
...
function p7AxisShouldShow() {
  ...
  // explicit instruction — it never waits for anything else to unwind).
  // Nothing else may show it early — p7HasEngaged flips true later on the same
  // card (fully out of view) and is what the FILL waits for, not the draw-in.
  if (typeof fold9AxisTrigger !== "undefined") return fold9AxisTrigger.target() > 0;
  return p7HasEngaged;
}
...
// The year marker's SHAPE on the vertical axis: 'ring' (the hollow dot) or 'line'
// (a short horizontal rule across the axis, centred where the ring sits).
// Length and darkness of the line are its own knobs. `var`s — a compare/ +
// manual/ harness drives them. Desktop-only in effect: mobile draws no year
// marker at all (P7_VERT_M.yearRing false).
var P7_AXIS_YEAR_MARK       = 'ring';
var P7_AXIS_YEAR_LINE_LEN   = 8;      // px, the whole line
var P7_AXIS_YEAR_LINE_ALPHA = 0.22;   // unreached; a reached year's line is P7_AXIS_FILLED_COLOR
...
function p7DrawYearAxisVertical(ctx, W, H) {
  ...
  const fillOriginY = segTop;   // where the fill RESTS — the raised first ring's centre (see fillY)
  ...
  // The fill RESTS at the first ring's centre (fillOriginY), not at topY: the
  // ring is raised P7_VERT_FIRST_YEAR_RAISE_PX above the line's top, so an edge
  // parked at topY painted that stub as reached the moment the fill got any
  // target at all — a 14px jump on the trigger frame, visible now that the
  // axis draws in half a fold before it engages (p7PreStartDate). The edge
  // crosses the stub FIRST, sliding from fillOriginY to topY over the first
  // `stubFrac` of lagged fill, and is exactly curY from there — continuous
  // both ways, so the retreat slides back into the ring the same way. At rest
  // (fillFrac 0) it sits on the ring and paints nothing.
  const stubFrac = len > 0 ? (topY - fillOriginY) / len : 0;
  const stubK    = stubFrac > 0 ? Math.min(1, fillFrac / stubFrac) : 1;
  let fillY = fillOriginY + (curY - fillOriginY) * stubK;
  ...
  for (const m of marks) {
    ...
    // The first year's ring is row 0, so at rest (curRow 0) `row <= curRow`
    // would light it before anything has been reached — it lights once the
    // fill edge has actually left it (fillY past its centre, see stubK).
    const reached = row === 0 ? (curRow > 0 && fillY > m.yc) : row <= curRow;
    const ringColor = hoverActive ? P7_AXIS_BG_COLOR : (reached ? P7_AXIS_FILLED_COLOR : P7_AXIS_BG_COLOR);
    if (ring && P7_AXIS_YEAR_MARK === 'line') {
      // A short horizontal rule instead of the ring: same spot, same
...
// Hover tooltip for a single event square in the real timeline (#page-9) — date
...
function p7HoverInit() {
  ...
  // Full clear (square + axis) — for leaving #page-9 entirely.
  function hide() {
    setAxisHover(null);
    hideSquare();
  }
...
/* =========================================================================
   MOBILE EVENT PICKER (#page-9 only) — the touch counterpart to p7HoverInit
   =========================================================================
   ...
   resting content is the line of text that names it:
     ...
     event — the ordinary docked tooltip (date + description). There is no
             ...
             it, or until leaving #page-9 releases the frame.
...
// change and then freeze — and on RELEASE the dim would be stranded at 1, the
// whole grid left dimmed with nothing picked.
//
// This is that loop: it runs only while a ramp is actually mid-flight, in either
// direction, and stops the frame it settles.
let p7PickRampRaf = 0;
function p7PickRampActive() {
  if (typeof p9 === "undefined") return false;
  ...
  // p7BulgeT at t > 0, where p7BulgeActive would keep reporting it forever.
  return p7BulgeT.size > 0 || dim > 0;
}
function p7PickRampTick() {
  p7PickRampRaf = 0;
  draw();
...
function p7StartPickRamp() {
  ...
  // 14k+ dots per frame — which is the stutter this loop exists to avoid.
  if (!p7PickRampActive()) return;
  if (!p7PickRampRaf) p7PickRampRaf = requestAnimationFrame(p7PickRampTick);
}
...
function p7InspectPage() {
  ...
  return (p7TimelineLive() || currentPage === 13) ? currentPage : null;
}
...
let p7BandEverTyped = false; // the axis has finished its build-in at least once
...
function p7HintBandApply() {
  ...
  // THE LINE IS ITS OWN LITTLE ANIMATION, in both directions — it never blinks
  // on or off. One displayed value (p7BandT) walks toward a target each frame:
  ...
  const tin = bandWant ? p7HintTypeT() : 0;
  if (tin >= 1) p7BandEverTyped = true;
  const target = bandWant ? (p7BandEverTyped ? 1 : tin) : 0;
  const now = performance.now();
  const dt = p7BandLastFrame === null ? 0 : now - p7BandLastFrame;
  p7BandLastFrame = now;
  if (p7BandT !== target) {
    const ms = target > p7BandT ? P7_BAND_TYPE_MS : P7_BAND_UNTYPE_MS;
    const step = dt / Math.max(1, ms);
    p7BandT = target > p7BandT ? Math.min(target, p7BandT + step)
                               : Math.max(target, p7BandT - step);
    // Its OWN frames: past the timeline nothing else repaints this — page7's
    // draw loop has stopped — so the walk has to drive itself or it freezes
    // mid-sentence.
    requestAnimationFrame(p7HintBandApply);
  } else {
    p7BandLastFrame = p7BandT === target && target === 0 ? null : p7BandLastFrame;
  }
  const t = p7BandT;
  el.hidden = t <= 0;
...
function p7InspectInit() {
  ...
  function sync() {
    ...
    tipEl.classList.toggle("is-hint", !hasEvent && currentPage === 13);
  ...
  function p7AxisTapHit(cx, cy) {
    // Same stale-map trap as the x-ray above: off the timeline folds these
    // positions are last frame's, from a fold that isn't on screen.
    if (!p7AxisCardsOnThisFold()) return null;
    const rect = canvasEl.getBoundingClientRect();
    const mx = cx - rect.left, my = cy - rect.top;
    for (const [ev, pos] of p7.axisEventPositions) {
      const dx = mx - pos.x, dy = my - pos.y;
