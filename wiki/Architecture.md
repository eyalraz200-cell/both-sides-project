...
| Mini-legend + ACLED note | Six DOM group rows over the canvas; the note sits above their top row | **Both collapse into the מקרא card** — a floating card styled like the desktop note (tint, no border, no chevron, no handle; title row = the מקרא button), pinned top-right with its right edge flush to the title blocks, that opens width-then-height into the legend + note; the six rows fly into it at `@fold4` and it **closes itself** shortly after they land, the ACLED note joining it a fold later. See [Groups-and-Legend](Groups-and-Legend.md#the-mobile-מקרא-bar) |
