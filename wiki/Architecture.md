...
| Mini-legend + ACLED note | Six DOM group rows over the canvas; the note sits above their top row | **The legend collapses into the מקרא card** — a floating card styled like the desktop note (tint, no border, no chevron, no handle; title row = the מקרא button), pinned top-right with its right edge flush to the title blocks, that opens width-then-height into the six group rows; they fly into it at `@fold4` and it **closes itself** shortly after they land. **The ACLED note has no mobile counterpart** — the credit is a bare `acleddata.com` link in the opposite top-left corner, on the same `@fold6` crossing. See [Groups-and-Legend](Groups-and-Legend.md#the-mobile-מקרא-bar) |
