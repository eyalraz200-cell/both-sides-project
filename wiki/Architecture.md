...
| `#page-12` (outro) frame / title | sized from the viewport edges: `height: calc(100vh - 96px)` (48px gap top and bottom), width 520px = the narrowest frame the copy (padding included) fits at on a 982px-tall viewport, 40px side padding, 42px top/bottom, copy vertically centred in the leftover height (`#page-12 .text-card` is `fit-content` so it stays centred) / 40px | `min(450px, 100vw-48px)` border-box, height auto / 28px |
