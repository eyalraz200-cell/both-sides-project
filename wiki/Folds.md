...
`PAGES[]` in `js/core.js` — there are **15 folds** (`page-0` … `page-15`), matching the
15 entries in `PAGES[]` and the 15 `.text-section`s in `project.html`.

> Older notes describe 11–14 folds. All are stale — the code has 15, ending at
> `#page-15`. This page is the source of truth.
...
| `@fold7` | `page-6` | `drawFold7` | כל ריבוע מייצג פעולה פוליטית שהתרחשה ותועדה במרחב הציבורי. | Two crossings, both at 0.5: `fold7LabelTrigger` types the square labels in and `fold8SquareDimTrigger` dims the 7 non-demo squares, so the demo dot is **already singled out here** (explicit instruction). Only the tooltip and the swell moved to @fold8 |
| `@fold8` | `page-7` | `drawFold7` | ריחוף העכבר מעל ריבוע חושף את תיאור הפעולה וממחיש את סדר גודלה. | **The hover demo**, same canvas as @fold7. One crossing on desktop (the dim already happened on @fold7): once the card's bottom has reached the tooltip's own top edge plus `FOLD8_TOOLTIP_CLEARANCE_PX` (**−20px** — negative, so the card may still overlap that edge by 20px when the fold fires), `fold8TooltipTrigger` grows + types the demo tooltip (date + description, unchanged animation) and `fold8DemoGrowTrigger` swells square 0 from its 8px rest side to `FOLD8_DEMO_GROW_PX` — «סדר גודלה», the two reading as one gesture. Each beat waits its own dead time after that one crossing before starting — the swell leads (`FOLD8_DEMO_GROW_DELAY_MS` 0, over `FOLD8_DEMO_GROW_MS` 350ms) and the tooltip follows 250ms later (`FOLD8_TOOLTIP_DELAY_MS`, a delay at the head of its wall-clock sequence, then `FOLD8_GROW_MS` 400ms of grow-and-hold; the swell's delay is a `{start, len}` window on the swell trigger's raw progress read through `fold8DemoGrowT()` — never `fold8DemoGrowTrigger.currentT()`, which still carries the delay). The swell is **centred on the dot** (`growOffset` in js/update-groups.js shifts the square back by half the extra size, since `.fold6-square` is anchored top-left on its zero-size wrap; the offset fades out with `resizeT` so the fly's own geometry is untouched), and the tooltip is positioned off the square's **edges**, not its centre, so its 5px gap is constant at any size. The grown side is the size the @fold9 fly then resizes *from*, so the square never snaps back before departing. Fully reversible (**mobile:** the tooltip is a docked frame above the timeline instead — see [Timeline](Timeline.md#hover)) |
| `@fold9` | `page-8` | `drawFold9` | צבע הריבוע מציין את הקבוצה שביצעה את הפעולה.<br>ניתן לסנן קבוצות באמצעות המקרא. (two lines, a hard `<br>` — the only title block with one) | `fold9Trigger` colors square 0 + its tooltip (desktop: the fill; mobile: the dashed border); `fold9FlyTrigger` colors all 8 and flies them to their real per-event dots — the 4 earliest events per side, pinned by row id (`FOLD6_SQUARE_ROW_IDS`, see [Groups-and-Legend](Groups-and-Legend.md#the-8-sample-squares)). **Desktop:** the demo callout swings from hanging above square 0 to hanging below it (`.is-flipped`) as that square travels, so it is angled down when it lands on the timeline — driven by `fold8FlyMoveT`, square 0's beat-2 progress, **not** by `fold9FlyTrigger` as a whole (whose first `FOLD9_FLY_RESIZE_SPAN` is color+resize, before the square has moved) |
| `@fold10` | `page-9` | `drawPage7` | *(no title — `page7-scrub`)* | The pinned real timeline — desktop: vertical centre axis, rows are dates; mobile: horizontal bottom axis — see [Timeline](Timeline.md). Legend rows are clickable here and only here: a click filters that group out of the graph (`p7FilterToggle`, desktop only) |
| `@fold11` | `page-10` | `drawPage7` | המחנות נבדלים במספר הפעולות שביצעו, אך פעולה אחת עשויה להתבצע בידי אדם יחיד, ואחרת בהשתתפות מאות אלפי אנשים. | **The size grid.** The same `drawPage7` canvas with `p7Grid.on` — the trigger is this card's top crossing **mid-screen**, the house 0.5 (`checkFold10Grid`, js/groups.js — a `watchCardThreshold` shim over `p7SizeGridSet`; desktop only, the grid is forced off on mobile, and `p7SizeGridOnPage` in page7.js only re-syncs it on re-entry from below and snaps it off past @fold12). Every dot on screen morphs to its crowd tier (`P7_GRID_TIER_CELLS`) and re-packs into a gap-exact block per camp while the year axis and the timeline undraw the way they already do off @fold10. One choreographed ms clock (`p7MorphTotalMs()`: flight, then size in tier waves) on `p9Ease`, blending centre **and** size — **nothing fades**; hover tooltip + dim work here. Scrolling back to @fold10 plays the exact mirror; **downward the grid stays on** — @fold12's bridge glide flies the dots out of it (see that row). Geometry and the baked values in [Timeline](Timeline.md#the-size-grid) |
| `@fold12` | `page-11` | `drawPage8` | פעולות פוליטיות נבדלות זו מזו במטרה, באמצעים ובאופן הפעולה. אבל הגבול בין פעולה לגיטימית לפעולה קיצונית אינו תמיד מוסכם. | Bridge glide into page9's legit grid (`page8.js`) — **it starts from @fold11's size grid, not from the timeline**: each dot leaves its packed cell at its own tier size and shrinks to the legit size across the flight (`p7GridRestRect`, page7.js), so nothing snaps back onto the timeline in between. `p7SizeGridOnPage` keeps `p7Grid.on` for this page alone; @fold13 snaps it off. The card is **centred**, and the section is **125vh with `padding-bottom: 25vh`** on desktop (`@media (min-width:601px) #page-11`). That pair is one decision: the padding shrinks the centring box back to the first 100vh, so the card still sits at 50vh — the house **100vh from @fold11's card centre** — while `#page-12` starts 125vh in, which is the house 100vh on to @fold13's card, because @fold13's card is **not** centred in its section (it centres 21vh into the desktop 50vh `.page9-title-row`, 29vh higher than 50vh). **Don't top-align this card** (`align-items`/`padding-top`): it used to be `flex-start` + `padding-top: 48px`, a leftover from when the 780vh scrub sat directly above it with no @fold11 card filling that first half-viewport — with @fold11 in place that left the two title blocks **0.65vh** apart. And **don't trim the height** (60vh was tried: both cards parked on screen together) |
| `@fold13` | `page-12` | `drawPage9` | מה נחשב בעיניכם לפעולה לגיטימית, ומה לפעולה קיצונית? | Headline is forced to **one line** at ≥640px (`.page9-title-row .text-card` `width: max-content` + `white-space: nowrap` on the frame, style.css) — it wraps at the 480px `--card-w`; under 640px it wraps as normal. Drag-and-drop categorization. **Desktop section is 150vh, not the house 200vh** (`.page9-title-row` 50vh, sticky still 100vh): the panel engages when the title card sticks and the @fold14 gate is one viewport before `#page-13`, so the 100vh row left ~55vh of pinned, motionless panel before the closing card could rise; the card now sticks ~17vh in and the gate is at 50vh — see [Drag-and-Drop](Drag-and-Drop.md). Desktop runs **layout V2** (`P9_LAYOUT_V2`): bare pill band on top (its pills **pop in right → left** on `.engaged`, the band's rule drawing under them in step), horizontal drop zone under it, pills classified by **drag or click**, legit dots denser in a 150px bottom strip — see [Drag-and-Drop](Drag-and-Drop.md#desktop-layout-v2) |
...
**@fold9 — the 8 squares fly to real dots, permanently.** `fold9FlyTrigger`
...
@fold9/@fold10 rest is at the *bottom* of the viewport — `tooltipDockRestPx()`, see
...
the @fold7→@fold9 dock lerp, which a transition would smear.
...
equivalent: the mobile event picker on `@fold10` (see
