...
| `@fold6` | `page-5` | `drawFold7` | כל ריבוע מייצג פעולה פוליטית בשטח | `fold7LabelTrigger`: square labels appear; `fold8SquareDimTrigger` dims the rest while the fold-8 tooltip grows + types (**mobile:** the tooltip is a docked frame above the timeline instead — see [Timeline](Timeline.md#hover)) |
...
so @fold10's mobile stack reads top-to-bottom as **legend → title → pill band → docked tooltip
...
has sat since @fold7 (`TOOLTIP_DOCK_TOP_PX` 62), so a third dock spot was added:
...
fold **early** — from `page8CheckScroll`'s @fold9 title crossing (`js/page8-9-scroll.js`),
...
the @fold6→@fold7 dock lerp, which a transition would smear.
...
On @fold11's exit (`js/fold11.js`) the tray does not slide out — it fades in place with
...
`keepEmptyFrame` lifetime bound (`js/update-groups.js`) is `currentPage <= 10`, not 9:
the IntersectionObserver flips `currentPage` to 10 partway through @fold11's scroll-in,
...
and pitch the dots had in @fold10's extreme grid (1.5px on 2px there); desktop keeps its
original `P7_CELL` spread pitch with 3px dots.

Hover-only affordances (the @fold9 square dim, the axis hover states) simply never activate
...
equivalent: the mobile event picker on `@fold8` (see
