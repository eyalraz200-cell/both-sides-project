...
| `@fold12` | `page-11` | `drawPage12` | בתקופה של מחלוקות עמוקות, חשוב שנסתכל מעבר למטרות הפוליטיות של שני המחנות, אלא גם על האמצעים להשגתן: אילו פעולות אנחנו מוכנים לקבל במרחב הציבורי, ואילו פעולות חוצות את הגבול. | The closing statement — **this**, not the credits card, is what @fold11's interaction resolves into. It owns the scroll gate (`p13GateLocked`/`p13GateMax`, unlocked once one @dragcard is extreme; **`let p13TouchBlockOn` must stay declared above the eager `p13SyncGateVisibility()` call** — below it, that call hits the `let` in its temporal dead zone, the throw kills the rest of js/fold11.js, and the wheel/keydown/scroll listeners never register, so the gate silently does nothing and @fold13/@fold14 can be reached with no pill dropped) and **both halves of the hand-off, run back to back across its card's rise**. First the scroll-linked fade-out (`fold13ScrollT`): tray, header, extreme zone, legend, ACLED note, legit dots and counts all fade (and the whole `.frozen` fixed panel goes `pointer-events: none` while faded — it never leaves the screen, and its invisible tray band was eating the hover on @fold13's share buttons) — but the **extreme dots stay standing in their columns, and the camp dividing line stays with them** (it's what those columns sit on; the line rides `fold13ExtremeMorphT`, not `fold13OutT` — see `page9.js`). That fade is compressed into the **first half** of the rise by `FOLD13_FADE_SPAN` (0.5, js/fold11.js), so it is finished by the time the card reaches mid-screen. Then, at exactly that point, `checkFold13` fires the **freeform spread** onto an otherwise-clear screen (`fold13Trigger`, frac 0.5 on this fold's own wrapper). `FOLD13_FADE_SPAN` and that frac are **one decision in two files** — raise the span and the fade runs under the spread. The section's height is **solved**, not fixed: `p12SpacingFit()` (page12.js, run at load, on `fonts.ready` and on resize) sets `#page-11`'s min-height to **50vh + half this card** (50vh is the no-JS fallback in style.css), and its `.page12-sticky-center` is `position: static` (the shared wrapper's sticky 100vh would otherwise hold the section open regardless). That is the one value that puts @fold13's centred card exactly **100vh below this card's centre** — the same centre-to-centre distance as every other consecutive pair of title blocks, whose cards are each centred in a 100vh section. **Never replace it with a fixed vh** — 45vh, 80vh, 100vh, 125vh and a 100vh `padding-bottom` were all tried and each was off by exactly the half-card it ignored. **@fold12 waits directly under @fold11, with no empty run between them, at every width.** The card is pulled flush to the section top (`align-items: flex-start` + `padding-top: 0`). That zero is load-bearing: the gate holds scroll at `#page-11.offsetTop - 100vh` and can't move later (any later and the section is on screen while still locked), so only a card at the very top of its section has its first pixel at the viewport's bottom edge at the exact scroll position the gate unlocks. **Any `padding-top` here re-opens the gap by its own height** — an earlier 25vh did exactly that, and the symptom is easy to misread, because @fold11's own panel is `.frozen` (pinned at top) through the hand-off: with the card still below the fold, the only thing happening for that stretch is an opacity crossfade on a motionless panel. **The fade IS the card's rise:** `fold13ScrollT` measures from the moment the card's top clears the viewport's bottom edge (now the gate line itself) to the moment it reaches its resting spot (`scrollY = #page-11.offsetTop`, flush at the viewport top), and runs on **`p9Ease`** (sine in-out), which finishes at `t=1`, exactly where the card lands. The outro card (@fold14, then directly after this one) is near-full-height, so at the house 100vh it was already climbing into view while this card sat mid-screen. **The two changes are a pair** — extra height under a *sticky* wrapper is hold time, not spacing (a 200vh sticky attempt read as the card being stuck dead centre). None of the scroll machinery cares: `p13GateMax`, `fold13ScrollT` and the `.frozen` threshold all measure `#page-11`'s `offsetTop`, and nothing needs this wrapper to pin any more (`checkFold13` moved off the outro's). An ordinary `.section-title.text-card-frame` title block, wrapped in `.page12-sticky-center` only because `page12StickyEl` — the `gate-hidden` toggle (`p13SyncGateVisibility`) — hangs off that wrapper |
| `@fold13` | `page-12` | `drawFoldMap` | (no title) + שתפו עם אחרים | **The event map is DRAWN in, on the shared canvas, behind the share block.** No map markup at all — `drawFoldMap` (map.js) is `PAGES[12]` and `PAGES[13]`; the section is an ordinary house fold (100vh, share card centred). The build is a **fold trigger**, not a scrub: `foldMapTrigger` (js/groups.js, `makeTrigger(FOLD_MAP_MS)`, 5200ms — a named exception to `GROUP_TRANSITION_MS`, three beats over 14k dots) fires when the share card (`#page-12 .page12-share-card`) crosses the house **0.5** line (`checkFoldMap`), and is reversible like every other trigger. Its tick writes the raw progress into `p12map.t`. Three beats, each a `{start, len}` window over that raw `t` with `p9Ease` applied fresh: **outlines wipe on** (`MAP_WIPE` 0–0.34: each Natural Earth ring is dashed `[len·k, len]` so it strokes itself; the country fills fade up a beat behind, `MAP_FILL` 0.18–0.44; Israel/Palestine darker + `#999`, the wider region — Egypt, Jordan, Lebanon, Syria, Saudi Arabia, Iraq, Turkey, Cyprus, Northern Cyprus, Kuwait, Sudan, Libya — pale + `#ccc`, since the map is viewport-wide), **the extreme dots FLY** (`MAP_FLY` 0.30–0.72: the dots @fold12 left standing in their freeform spread — `p9.leftTopOrder`/`rightTopOrder` — lerp from `p12EnsureFreeformTargets` to their own map cell, shrinking from `p9Metrics().SQ` to the map dot size on the way; at t = 0 they are drawn where @fold12 left them, so the hand-off is seamless, and until the map data has arrived `drawFoldMap` just defers to `drawPage12`), then **the legit dots POP** (`MAP_POP` 0.55–1: every other event scales up in place at its cell, each with a fixed hashed stagger over the first 60% of the window, `MAP_POP_STAGGER`). City labels (`MAP_CITIES`, 16 Hebrew names in three tiers) fade in at t = 0.62 / 0.72 / 0.82. **Every event has its own cell**: events geocode to settlement centroids (904 coordinates for 14,451 rows, `map/event-points.json`). All tunables live in one object, **`MAP_TUNE`** (map.js top): `dot` (0.5px at zoom 1), `dotGrow` 0.65 (the dot is `dot × z^dotGrow`, capped — 1 grows with the map, 0 never grows), `dotMax` (6px), `gapRatio` (gap = dot × 0.45), `cellBase` (46px cluster bucket — screen px, so clusters re-form as the reader zooms, or KM on the ground with `cellKm` on, which keeps the same coordinates together at every zoom), `cityKm` (0 = off; when > 0 any point within it of a labelled city joins that city's cluster), `cluster` — **the bucketing method**: `grid` (square buckets, the default), `hex` (hex buckets, no cell-edge seams), `link` (greedy single-link: biggest places seed, everything else joins the first seed within `linkPx`), `city` (every point to its NEAREST labelled city — 16 blobs), `none` (one blob per coordinate — 904); `minCluster` (clusters under that many events fold into their nearest neighbour, smallest first; 0 = off), `jitter` (a deterministic per-dot wobble as a share of the pitch, 0 = a clean lattice), `counts` (write cluster counts at all — the cutoff itself belongs to the zoom stop, see **`MAP_ZOOM_LEVELS`** below). **The cutoff is applied at DRAW time, not at layout time**: `marks` holds every cluster's centroid + size, so the count knobs act with no re-pack and the numbers thin out live during a wheel gesture, off the live `p12map.view.z` rather than the held layout's — `p12MapDrawCounts`, white-haloed 11px like the city labels, from t = 1), `shape`, `round`, `sort`, `morphMs`, `hoverPad`, `hoverDim`, `zoomLevel`, `startX`, `startY` and `roadW`. **The zoom is not continuous — it is FOUR stops, `MAP_ZOOM_LEVELS` (map.js, right under `MAP_TUNE`).** Each stop is one row — `z` (1 = the fit-to-height view), `countMin` (a cluster is labelled from this many events up), `cities` (how many `MAP_CITIES` tiers are up: 1 = the four majors only … 3 = all), `towns` (`MAP_DETAILS.labels`' small towns), `roads` (`MAP_DETAILS.roads`' highways) and `hover` (the dot tooltip answers) — so everything that used to be its own zoom threshold (`countZ1`/`countZ2`, `cityZ2`/`cityZ3`, `townZoom`, `hoverMinZoom`, `roadMinZoom`) is read off the stop instead. Currently `{1, 150, 1 tier}` · `{2.35, 75, 3 tiers, roads}` · `{6, 120, 3 tiers, towns, roads, tooltip}` · `{16, …same}`. `p12map.view.lvl` is the stop the view is parked on and `MAP_TUNE.zoomLevel` (1-based) the **resting** stop the map is built at — 2 today. **Removed — don't reintroduce:** the per-threshold zoom knobs above, and `zoomMin`/`zoomMax`/`zoomRate` (the stops ARE the range). Every colour the map paints lives in **`MAP_COLORS`**, read at **draw** time (`p12MapFill` resolves a country's fill per frame — the layout stores only the name and `main`), so a palette change lands on the next repaint with no re-pack (`bg`, `israel`, `palestine`, `neighbour`, `gaza`, `border`, `borderFar`, `water`, `waterEdge`, `city`, `town`, `count`, `road`) — the dots keep their `GROUPS` colours and are not in it. `MAP_DETAILS.roads` draws **`MAP_ROADS`** — 8 hand-traced polylines for routes 1, 2, 4, 6, 90, 40, 60 and 31. They are **approximate at map scale**, not survey geometry; the repo has no road data. Tuned by eye with a `manual/` harness on 2026-09-07. Clusters are seeded by that bucketing and then handed to a **packer**, `MAP_PACKERS[MAP_TUNE.shape]`: `square` (a ceil(√n)-sided grid per cluster centred on its centroid, overlapping squares merged repeatedly) or `grow-hex` / the default **`grow-square`** (**growth packing on a region mask**) or `disc` (a sunflower spiral per cluster — the i-th dot at angle i×137.5° and radius ∝ √i, so a cluster reads as a round blob of even density with no lattice rows; like `square` it ignores the coastline). The mask (`p12MapRegionMask`) rasterises Israel and Palestine once per layout, at the lattice pitch, into a byte per cell (1 = Israel, 2 = Palestine, 0 = sea/neighbours). Each cluster, largest first, then **grows cell by cell from its seed** into that shared lattice (a min-heap on distance² to the seed, `p12MapPackGrow`), claiming only cells in its own region; a seed off land is moved to the nearest land cell first. **`MAP_TUNE.crossBorder` (ON) makes pass 0 treat Israel and Palestine as ONE region**, so a blob straddling the green line grows straight through it and the two countries share one pool of cells; off, each blob stays in its seed's country. So blobs hug the coast, neighbouring clusters abut instead of overlapping, and **a dot never crosses the coast or a neighbour's border** (nor the green line with `crossBorder` off) — unless its region is genuinely full: growth widens in three passes (own region → any land → anything), and at zoom 1 Israel holds only ~12.4k hex cells for 14.5k events, so the country simply fills. Events are ordered by `p12MapSortByColor` and zipped onto the claimed cells: with `MAP_TUNE.sort` **`mix` (default)** the events are shuffled deterministically (hash of the row id), so the groups intermingle inside every cluster; `wedge` / `ring` colour-sort them instead and sort the cells by angle around the seed (colour wedges) or by distance (concentric rings). `round` (off) draws each dot as a circle (one `arc` path per colour) instead of a `fillRect`. Equirectangular, cos(mid-lat) corrected, lat 29.45–33.35 fit to canvas height at zoom 1 — zoom/pan exist but only past @fold14 (see there). Layout is cached per canvas size + zoom + extreme-roster size; dots are pre-grouped by color so a frame is six `fillStyle`s and one path (or a run of `fillRect`s) per colour. Data is fetched lazily when the section is within two viewports (`p12MapPrefetchCheck` → `p12MapLoad`). Then the share block: a plain `.text-card-frame` card (`.page12-share-card`, rtl, centred): the «שתפו עם אחרים» `.section-title` (the shared 20px/16px rule, no override) over the button row `.page12-share` (`#page12Share`: WhatsApp / X / Facebook intent links + copy-link, wired by `p12ShareInit` in page12.js; centred flex row, 8px gap, 16px under the title; the buttons invert **instantly** on hover, no transition; **no hairline**). The dots' flight and the map's build are one trigger, fired by this card. |
| `@fold14` | `page-13` | `drawFoldMap` | קיצוניים משני הצדדים (outro) | Outro/credits card as a **drawer** (desktop, `p13Drawer` in page12.js): `#page-13 .text-card` is `position: fixed` at the viewport's bottom edge (style.css, `min-width: 601px`), and JS writes its `translateY` every scroll frame from the distance left to the document's end, so it rises 1:1 with the scroll exactly like an in-flow card and **docks with only its title («קיצוניים משני הצדדים») showing** — the `peek`, measured as frame top → title bottom + 20px — at the last scrollable pixel. **Hovering the card slides it fully up** (`P13_OPEN_MS` 420ms, `p9Ease`, a lazily-built `makeTrigger` — js/groups.js loads after page12.js); leaving slides it back. The transform is JS-repainted, so it has **no CSS transition**. Its `.page12-sticky-center` is `position: static; height: auto` at every width, and `p12SpacingFit()` **solves the section height**: **(50vh − half the share card) + the peek**, so the card's top enters the viewport at the house edge-to-edge distance under the share card and the document ends exactly at the dock (100vh is the no-JS fallback; the finished map — `drawFoldMap`, `foldMapTrigger` settled at 1 — fills the viewport behind). Drag-pan ignores pointerdowns inside the card. At the dock a **back button** (`#p13Back`, project.html, a direct `.layout` child; `.p13-back` in style.css) appears centred at the top edge: a 48px circular title-block frame (white fill, the same 2px «2 2» dash as an SVG ring, up arrow; inverts on hover). Click resets the zoom (`p12MapResetView`) and smooth-scrolls one viewport back up. `p13Drawer.place()` toggles its `hidden` — shown only when nothing is left to scroll. **At the dock, the wheel zooms the map** (`p12MapWheel`, map.js, gated by `p12MapAtEnd`: `currentPage >= 12` — not 13, since the section is now only padTop + peek tall and never crosses the observer's centre line — data ready, `p12map.t ≥ 1`, `scrollY` at the document max, desktop only): **the wheel STEPS between the four `MAP_ZOOM_LEVELS` stops, one stop per gesture, and never lands in between** (`p12MapGoLevel`): wheel-up steps in about the cursor, wheel-down steps out; a trackpad flick is dozens of events, so a step arms a `MAP_STEP_MS` (320ms) cooldown and the rest of the flick is swallowed. Landing on the **resting** stop (`MAP_TUNE.zoomLevel`, 2) re-centres via `p12MapResetView` instead of holding the cursor point, so backing off always gives the same view: that stop's `z` with the fit-view point `MAP_TUNE.startX`/`startY` (0.5 / 0.385) under the viewport's centre. At stop 1 a wheel-down is not intercepted (there is nothing left to scroll anyway). The wheel never scrolls the page back up from the dock — that is the back button's job. Dragging pans whenever there is somewhere to pan (`p12MapCanPan`: zoomed in, or the docked drawer covers part of the map — the pan may push the map up by the drawer's peek height, `p12MapClampPan`; `grab`/`grabbing` cursor on the root element; while a dot is hovered the cursor is the normal arrow, `p12MapSyncCursor` yields to `p12map.hovered`); pan is clamped so the map always covers the viewport. The view is `p12map.view = {z, px, py, lvl}` (`lvl` = the index into `MAP_ZOOM_LEVELS`): `p12MapLayout` is rebuilt **at the zoom** (so clusters split as the reader zooms in, the city-radius and overlap rules re-run in screen px, and the dot grows `MAP_TUNE.dot × z^dotGrow` capped at `MAP_TUNE.dotMax`, gap `dot × gapRatio`), and the pan is a single `ctx.translate` in `drawFoldMap`). **Position never snaps on a zoom step**: when the layout is rebuilt at a new zoom on a finished map, every dot carries the cell it was showing in — its current on-screen spot, mid-glide included, scaled into the new zoom's px (`fx`/`fy`) — and glides to its new cell over `MAP_TUNE.morphMs` (600ms, `p9Ease`) on a self-driven rAF loop (`p12MapMorphStart`/`p12MapMorphK`); another wheel step mid-glide re-bases from the interpolated position. **The re-pack is held while the wheel is moving** (`p12MapZoomHold`, `MAP_ZOOM_SETTLE_MS` 120ms after the last event): mid-gesture frames draw the last layout scaled by `view.z / L.z` (`zs` in `drawFoldMap`; outline width and text stay unscaled) with hover off, then one re-pack + glide runs — a pack is 100–350ms and a trackpad fires dozens of events a second. Resize and roster changes rebuild without a glide. **Every dot is a hover target on the finished map from the first stop whose `hover` is on (stop 3 — zoomed out the dots are too small to aim at, so hover is off at stops 1–2)** (`p12MapHoverInit`, map.js — desktop, `t ≥ 1`, `currentPage` 12/13, not mid-drag): the nearest dot within `dot/2 + MAP_TUNE.hoverPad` px of the pointer shows the shared `#page9Tooltip` (date + `descHeMedium`, group-tinted via `setTooltipColor`; opens right of the dot, mirrored left when it would not fit, flipped downward above the 295px line like the timeline's), and `drawFoldMap` dims every other dot to `MAP_TUNE.hoverDim` (0.2, the same value as `HOVER_DIM_OPACITY`; a snap like the timeline's) and redraws the hovered one on top at full alpha — same size, no ring (`p12map.hovered`). It sets the tooltip's inline opacity to 1 while shown, since @fold12's fade-out leaves the shared element at 0; a zoom re-pack clears the hover and re-tests on the next frame. The view resets to the fit whenever the build drops below 1 (reversed trigger) and on resize. No pinch/touch zoom — on mobile the card stays in flow (no drawer) and the document just ends after it. **No trigger of its own** — the freeform spread and the camp divider's fade both fire back on @fold12 (`checkFold13` watches `#page-11`'s wrapper), so the dots have already spread by the time this card rises. Under the methodology the card carries a four-line credits paragraph (`.page12-credits`: research/design, mentor, Shenkar thanks, data — the data line links out as `acleddata.com`; one `<br>` per line; centred, 14px/1.5, 0.04em tracking, 70% black, under a hairline divider). **The hairline sits in the middle of its gap** — a 38px gap split 19px above the line (`margin-top`) and 19px below it (`padding-top`). The card already fills a 900px-tall viewport, so `@media (max-height: 920px)` also trims that spacing (9px/9px — still centred). The share row used to sit above the credits with a matching hairline; it is @fold13 now |
...
| `foldMapTrigger` | `#page-12 .page12-share-card` (@fold13's share card) | **0.5** — house crossing. Plays the whole map build (map.js) over `FOLD_MAP_MS` 5200ms: outlines wipe, the extreme dots fly from @fold12's spread onto the map, the legit dots pop in. |
...
| `@fold14` | ✅ Outro card width + title size clamped; body copy 14px/1.55 and the wrapper (static at every width now) grows to `height: auto; min-height: 100vh` — the methodology text is taller than a phone viewport, so it scrolls instead of clipping. That override is scoped `#page-13 .page12-sticky-center` on purpose — @fold12 shares the class and must not grow. The wheel zoom is desktop-only; on a phone the document just ends on the finished map. |
