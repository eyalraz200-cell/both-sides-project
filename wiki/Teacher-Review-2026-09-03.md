...
- [x] **G2. @fold13 bridge (`page-12`)** *(Desktop: `#page-12` is 60vh.)* shows a mostly empty screen ("70–80% blank"). Reduce the
...
## H. @fold13 → @fold14 copy (`page-12`, `page-13`) 🔴
...
- [x] **H1. Make the question stand alone.** "איפה עובר הגבול בעיניכם?" currently depends on
  ...
  לגיטימית?". *(Done: @fold14's headline is now "מה נחשב בעיניכם לפעולה לגיטימית, ומה לפעולה
  קיצונית?" — self-contained, names both poles. @fold13's own card is unchanged and still
  carries the "…הגבול אינו תמיד מוסכם" setup.)*
- [x] **H2. @fold13→@fold14 transition 🟢.** "A bit clumsy but livable." Idea: pills enter from
...
## I. @fold14 drag-and-drop (`page-13`, `page9.js`)
...
- [ ] **I3. Drop the redundant hint on @fold14.** The "press and hold a dot" hint was already
  taught on the timeline; at @fold14 it's out of context and competes with the pills. Either
...
- [ ] **I4b. 📱 Feature discovery instead of ⓘ 🟢.** Mushon's alternative: open one category
  description by default when @fold14 arrives (with an X) so the user discovers that pills have
...
- [x] **I6. Accessibility check 🟡.** The «גררו סוגי פעולות…» subtitle looks too transparent —
  ...
  here").
- [ ] **I7. "There's a bug that fires every second" 🟡** — something in @fold14 logs/loops
...
## K. Outro (@fold15, `page-14`, `page12.js`) 🔴
...
- [x] **K2. Bottom line + share.** After the chart the reader needs an easy landing: a short
  ...
  @fold15 card; the bottom-line summary belongs to a future, not-yet-created fold.)*
