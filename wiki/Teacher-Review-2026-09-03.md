...
- [x] **G2. @fold12 bridge (`page-11`)** *(Desktop: `#page-11` is 60vh.)* shows a mostly empty screen ("70–80% blank"). Reduce the
...
## H. @fold12 → @fold13 copy (`page-11`, `page-12`) 🔴
...
- [x] **H1. Make the question stand alone.** "איפה עובר הגבול בעיניכם?" currently depends on
  ...
  לגיטימית?". *(Done: @fold13's headline is now "מה נחשב בעיניכם לפעולה לגיטימית, ומה לפעולה
  קיצונית?" — self-contained, names both poles. @fold12's own card is unchanged and still
  carries the "…הגבול אינו תמיד מוסכם" setup.)*
- [x] **H2. @fold12→@fold13 transition 🟢.** "A bit clumsy but livable." Idea: pills enter from
...
## I. @fold13 drag-and-drop (`page-12`, `page9.js`)
...
- [ ] **I3. Drop the redundant hint on @fold13.** The "press and hold a dot" hint was already
  taught on the timeline; at @fold13 it's out of context and competes with the pills. Either
...
- [ ] **I4b. 📱 Feature discovery instead of ⓘ 🟢.** Mushon's alternative: open one category
  description by default when @fold13 arrives (with an X) so the user discovers that pills have
...
- [x] **I6. Accessibility check 🟡.** The «גררו סוגי פעולות…» subtitle looks too transparent —
  ...
  here").
- [ ] **I7. "There's a bug that fires every second" 🟡** — something in @fold13 logs/loops
...
## K. Outro (@fold14, `page-13`, `page12.js`) 🔴
...
- [x] **K2. Bottom line + share.** After the chart the reader needs an easy landing: a short
  ...
  @fold14 card; the bottom-line summary belongs to a future, not-yet-created fold.)*
